...
//endmodifdg
  ...
  // xml file
  char xmlfile[] = "../../config/Myskim.xml";
...
// // skimming trigger ee + une paire ee candidate
...
// 
// // Passage trigger
//      if (  (ChannelName=="mumu" && ( (TriggerPassed_mumu_DATA)) ) || 
//            (ChannelName=="ee" && ( (TriggerPassed_ee_DATA)))  ) {
//      
...
// skimming  une paire ll de Pt et eta minimaux
     //Collection of selected objects
     vector<NTElectron> selElectronsNoIso = sel.GetSelectedElectronsNoIso();
     vector<NTMuon>     selMuonsNoIso     = sel.GetSelectedMuonsNoIso();
// At least one ee || mumu || emu

     if ( selElectronsNoIso.size()>=2 || selMuonsNoIso.size()>=2 || (selElectronsNoIso.size()+selMuonsNoIso.size())>=2 ) { 
        keepEvent = true;
	std::cout<<"event selectionne = "<<inEvent->eventNb<<std::endl;
     }	
