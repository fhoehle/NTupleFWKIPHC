...
//   double SF_DY_em = 1.00;
 ...
 //  TFile *f_data  = new TFile("../backup_outputProof22-05-12_12-17-40_evtSelAB/proof.root"); // to be uesed for the paper
    //TFile *f_data  = new TFile("backup_outputProof20-02-13_10-03-01/proof.root"); // to be uesed for the paper
    TFile *f_data  = new TFile("ProofNewSel/proot_ttbarMG_noDYSF.root"); // to be uesed for the paper

  
  
  //TFile *f_data  = new TFile("../backup_outputProof02-04-12_14-05-11_2011AB/proof.root"); // to be uesed for the paper
...
//  string ofilenametex = string("CrossSection")+string(".tex");
	...
	if( (k1 == 2 && k2 >= 4) || k1 == 1 ) TabFlow2[k0][k1][k2]= TabFlow2[k0][k1][k2];
	  ...
	  if (k1==0 || k1==2 || k1==4 || k1==5   ){       
	    TabFlow2[k0][k1][k2]= TabFlow2[k0][k1][k2] 
	  				+ pow(lumi_error*TabFlow1[k0][k1][k2], 2) 
  ...
  for(int k0=0; k0<5; ++k0) {
	...
	if(k1 != 3) TabFlow1[k0][50][k2] += TabFlow1[k0][k1][k2];
	if(k0== 2 && k2 == 6) cout << "process n " << k1 << " cutflow " << TabFlow1[k0][k1][k2] << endl;
      }
    }
  } 
  ...
  for(int k0=0; k0<5; ++k0) {
    for(int k2=1; k2<51; ++k2) {
      for(int k1=1; k1<10; ++k1) {
	if(k1 != 3) TabFlow2[k0][50][k2] += TabFlow2[k0][k1][k2]*TabFlow2[k0][k1][k2];
      }
