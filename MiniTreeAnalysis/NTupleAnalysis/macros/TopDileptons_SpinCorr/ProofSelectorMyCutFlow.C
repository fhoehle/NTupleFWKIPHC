...
ProofSelectorMyCutFlow::ProofSelectorMyCutFlow()
{
	...
	TupleTreeUnfolding = 0; 
	TupleTreeBeforeSelection = 0;

	tauPolReweighting = true;//false;
        topPtMcWeightFunc = 0;
	dorand = false;
	randsup = -1; // 1 or -1

	doTopReco      = false;//true;
	applyDYscale   = true;
	applyFakescale = true ;
	...
	doPDFreweight = false;
	//doPDFreweight = false;
	lhapdfFile = "/opt/sbg/data/data1/cms/jandrea/LHAPDF/lhapdf_versiontest/share/lhapdf/PDFsets/cteq61.LHgrid";
	...
	//initMCevents["TTbar"]	        = 3631452; // for top xs measurement
	//initMCevents["TTbarMCatNLO"]	= 20785270; //fullsim
	initMCevents["TTbarMCatNLO"]	= 10981220; //fullsim spin corr
	//initMCevents["TTbarMCatNLO"]	= 12011960; //fullsim no spin corr


	initMCevents["TTbar"]	= 3631452;
	//initMCevents["TTbar"]	=2918220 ;
...
//--------------------------------------------------------------------------------
TLorentzVector ProofSelectorMyCutFlow::getBoostedPionInWFrame(TLorentzVector part,TLorentzVector RestParticle){
	TLorentzVector boostedPart = part;
	TVector3 boostVec = RestParticle.BoostVector();
	boostedPart.Boost(TVector3(boostVec.Px()*-1.0,boostVec.Py()*-1.0,boostVec.Pz()*-1.0));
	return boostedPart;
}
//_____________________________________________________________________________
void ProofSelectorMyCutFlow::Init(TTree *tree)
{
	...
	cout << "end init tree " << endl;
        ...
        topPtMcWeightFunc = new TF1("topPtMcWeightFunc","TMath::Exp(0.199-0.00166*x)");
}
...
void ProofSelectorMyCutFlow::SlaveBegin(TTree * tree)
{
	...
	//--------------------------------------//
	//   Create top reconstructor 	
	//--------------------------------------//

	if(doTopReco) amwt = new AMWT(*anaEL, dataset->isData());
	...
	//double lumisf = 1./1.044;

	if(applyDYscale){ 


		//for TOP-11-005 paper
		/*
		//after dilept invMass
		vSF_DY_ee.push_back(1) ;
		vSF_DY_ee_error.push_back(0) ;
		//after jet cut
		vSF_DY_ee.push_back(1.120) ;
		vSF_DY_ee_error.push_back(0.074) ;
		//after met cut
		vSF_DY_ee.push_back( 1.847/vSF_DY_ee[1]) ;
		vSF_DY_ee_error.push_back(0.394) ;
		//for 1 btag 
		vSF_DY_ee.push_back(2.029/(1.847)) ;
		vSF_DY_ee_error.push_back(0.512) ;
		//for E btag 
		vSF_DY_ee.push_back(2.042/(1.847)) ;
		vSF_DY_ee_error.push_back(1.008) ;

		//after dilept invMass
		vSF_DY_mm.push_back(1) ;
		vSF_DY_mm_error.push_back(0) ;
		//after jet cut
		vSF_DY_mm.push_back(1.204) ;
		vSF_DY_mm_error.push_back(0.075) ;
		//after met cut
		vSF_DY_mm.push_back(1.745/vSF_DY_mm[1]) ;
		vSF_DY_mm_error.push_back(0.360) ;
		//for 1 btag 
		vSF_DY_mm.push_back(1.658/(1.745)) ;
		vSF_DY_mm_error.push_back(0.417) ;
		//for E btag 
		vSF_DY_mm.push_back(0.785/(1.745)) ;
		vSF_DY_mm_error.push_back(0.642) ;
		...
		//after jet cut
		vSF_DY_em.push_back(1.33) ;
		vSF_DY_em_error.push_back(0.28) ;
		//after met cut
		vSF_DY_em.push_back(1) ;
		vSF_DY_em_error.push_back(0.28) ;
		//for 1 btag 
		...
		vSF_DY_ee_error.push_back(0) ;
		//after jet cut
		vSF_DY_ee.push_back(1.141) ;
		vSF_DY_ee_error.push_back(0.073) ;
		//after met cut
		vSF_DY_ee.push_back( 1.654/vSF_DY_ee[1]) ;
		vSF_DY_ee_error.push_back(0.339) ;
		//for 1 btag 
		vSF_DY_ee.push_back(2.158/(1.654)) ;
		vSF_DY_ee_error.push_back(0.537) ;
		//for E btag 
		vSF_DY_ee.push_back(2.779/(1.654)) ;
		vSF_DY_ee_error.push_back(1.907) ;

		//after dilept invMass
		vSF_DY_mm.push_back(1) ;
		vSF_DY_mm_error.push_back(0) ;
		//after jet cut
		vSF_DY_mm.push_back(1.170) ;
		vSF_DY_mm_error.push_back(0.071) ;
		//after met cut
		vSF_DY_mm.push_back(1.532/vSF_DY_mm[1]) ;
		vSF_DY_mm_error.push_back(0.325) ;
		//for 1 btag 
		vSF_DY_mm.push_back(1.517/(1.532)) ;
		vSF_DY_mm_error.push_back(0.415) ;
		//for E btag 
		vSF_DY_mm.push_back(1./(1.532)) ;
		vSF_DY_mm_error.push_back(1) ;
		...
		vSF_DY_em_error.push_back(0) ;
		//after jet cut
		vSF_DY_em.push_back(1.46) ;
		vSF_DY_em_error.push_back(0.19) ;
		//after met cut
		vSF_DY_em.push_back(1) ;
		vSF_DY_em_error.push_back(0.19) ;
		//for 1 btag 
	...
	if(applyFakescale){  
		//after MET selection
		vSF_FakeBck_ee.push_back(22.0/19.9); 
		vSF_FakeBck_ee_error.push_back(12.0); 
		//after 1 b jet
		vSF_FakeBck_ee.push_back( (6./17.8) /vSF_FakeBck_ee[0]); 
		vSF_FakeBck_ee_error.push_back(12.7); 
		//after 2 b jet
		vSF_FakeBck_ee.push_back( (6./17.8) /vSF_FakeBck_ee[0]); 
		vSF_FakeBck_ee_error.push_back(12.7); 
		...
		//after MET selection
		vSF_FakeBck_mm.push_back(31.1/6.3); 
		vSF_FakeBck_mm_error.push_back(7.2); 
		//after 1 b jet
		vSF_FakeBck_mm.push_back( (24.2/4.7) /vSF_FakeBck_mm[0]); 
		vSF_FakeBck_mm_error.push_back(13.4); 
		//after 2 b jet
		vSF_FakeBck_mm.push_back( (24.2/4.7) /vSF_FakeBck_mm[0]); 
		vSF_FakeBck_mm_error.push_back(13.4);
		...
		//after MET selection
		vSF_FakeBck_em.push_back(194.9/42.8); 
		vSF_FakeBck_em_error.push_back(38.6); 
		//after 1 b jet
		vSF_FakeBck_em.push_back( (122.5/38.0) /vSF_FakeBck_em[0]); 
		vSF_FakeBck_em_error.push_back(36.2); 
		//after 2 b jet
		vSF_FakeBck_em.push_back( (122.5/38.0) /vSF_FakeBck_em[0]); 
		vSF_FakeBck_em_error.push_back(36.2); 
	...
	vSF_met_ee.push_back(1.);
	vSF_met_ee_error.push_back(0.);
	vSF_met_ee.push_back(1.);
	vSF_met_ee_error.push_back(0.);
	vSF_met_ee.push_back(1.);
	vSF_met_ee_error.push_back(0.);

	vSF_met_mumu.push_back(1.);
	vSF_met_mumu_error.push_back(0.);
	vSF_met_mumu.push_back(1.);
	vSF_met_mumu_error.push_back(0.);
	vSF_met_mumu.push_back(1.);
	vSF_met_mumu_error.push_back(0.);
	...
	if(datasetName=="TTbarMCatNLO") isTTbarMCatNLO	= true;
	if(datasetName=="TTbarMCatNLO") initMCevents["TTbar"]  = initMCevents["TTbarMCatNLO"];

	// cout << "initMCevents[\"TTbar\"]  " <<  initMCevents["TTbar"] << endl;
	...
	mcEventWeightHist = new TH1F("mcEventWeightHist","mcWeights",600,-300,300);
	TreeNotAlreadyFilled = true;
	TupleTreeUnfolding = new TTree("TupleTreeUnfolding","TupleTreeUnfolding") ;
        TupleTreeBeforeSelection = new TTree("TupleTreeBeforeSelection","TupleTreeBeforeSelection");
	TupleTreeUnfolding->Branch("deltaPhiGen",&deltaPhiGen,"deltaPhiGen/F");
	TupleTreeUnfolding->Branch("deltaPhiRec",&deltaPhiRec,"deltaPhiRec/F");
	TupleTreeUnfolding->Branch("totalEventWeight",&totalEventWeight,"totalEventWeight/F");
	TupleTreeUnfolding->Branch("ttbarGen",&ttbarGen,"ttbarGen/F");
	TupleTreeUnfolding->Branch("ttbarRec",&ttbarRec,"ttbarRec/F");

	TupleTreeBeforeSelection->Branch("deltaPhiGen",&deltaPhiGen,"deltaPhiGen/F");
        TupleTreeBeforeSelection->Branch("ttbarGen",&ttbarGen,"ttbarGen/F");
        TupleTreeBeforeSelection->Branch("totalEventWeight",&totalEventWeight,"totalEventWeight/F");
	nbins = 200;
	minx = 0.;
	maxx = 350;

	// load tau polarization histogram
	if(tauPolReweighting){
		TFile * tauPolReWFile = new TFile("/grid_mnt/opt__sbg__data__safe1/cms/fhoehle/CMSSW_Jeremy/TauPolSys_Studies/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/TopDileptons_SpinCorr/backup_outputProof_13-04-16_18-46-00_Mcatnlo5217_TauPolSys_FullSim_noTauPolReweighting_completeSample_tauWeights.root");//TauLeptonEnergyFraction_TauPol_mcatnlo5217_FullSim_small.root");
		tauPolReweightHistForMCatNLO = (TH1F*)tauPolReWFile->Get("tauPolLepWeights;1");
	}
	tauPolLepEnergyDist = new TF1("lepEtauPol","1.0/2.0*(5.0/3.0 - 3*x*x + 4.0/3.0*x*x*x + (-1.0/3.0 + 3*x*x - 8.0/3.0 *x*x*x))");
	//***********************
	...
	MyhistoManager.CreateHisto(HNjets_mumu_aftermetbtag1, "Njets_mumu_aftermetbtag1" ,datasetName,"Njets","Entries",4,0.5,4.5);
	MyhistoManager.CreateHisto(HNjets_emu_aftermetbtag1,  "Njets_emu_aftermetbtag1"  ,datasetName,"Njets","Entries",4,0.5,4.5);
	MyhistoManager.CreateHisto(HNjets_ee_aftermetbtag1,   "Njets_ee_aftermetbtag1"   ,datasetName,"Njets","Entries",4,0.5,4.5);
	...
	MyhistoManager.CreateHisto(testRand_ee, "testRand_ee", datasetName,    "testRand", "Entries", 1000, -50, 50);
	MyhistoManager.CreateHisto(testRand_mumu, "testRand_mumu", datasetName,"testRand",  "Entries", 1000, -50, 50);
	MyhistoManager.CreateHisto(testRand_emu, "testRand_emu", datasetName,  "testRand",  "Entries", 1000, -50, 50);
	...
	MyhistoManager.CreateHisto( DeltaPhiLLept_mumu_aftermetcut, "DeltaPhiLLept_mumu_aftermetcut",   datasetName,  "deltaPhi", "Entries",   10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_ee_aftermetcut ,  "DeltaPhiLLept_ee_aftermetcut",     datasetName,  "deltaPhi", "Entries",   10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_emu_aftermetcut,  "DeltaPhiLLept_emu_aftermetcut",    datasetName,  "deltaPhi", "Entries",   10, 0, 3.14);

	MyhistoManager.CreateHisto( DeltaPhiLLept_mumu_afterbtag1, "DeltaPhiLLept_mumu_afterbtag1",   datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_ee_afterbtag1,   "DeltaPhiLLept_ee_afterbtag1",     datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_emu_afterbtag1,  "DeltaPhiLLept_emu_afterbtag1",    datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted, "DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted",   datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_ee_afterbtag1_tauPolReweighted,   "DeltaPhiLLept_ee_afterbtag1_tauPolReweighted",     datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_emu_afterbtag1_tauPolReweighted,  "DeltaPhiLLept_emu_afterbtag1_tauPolReweighted",    datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
        MyhistoManager.CreateHisto( DeltaPhiLLept_topPtReweighted_emu_afterbtag1,  "DeltaPhiLLept_topPtReweighted_emu_afterbtag1",    datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
        MyhistoManager.CreateHisto( DeltaPhiLLept_topPtReweighted_ee_afterbtag1,  "DeltaPhiLLept_topPtReweighted_ee_afterbtag1",    datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
        MyhistoManager.CreateHisto( DeltaPhiLLept_topPtReweighted_mumu_afterbtag1,  "DeltaPhiLLept_topPtReweighted_mumu_afterbtag1",    datasetName,  "deltaPhi", "Entries",    10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_mumu_afterbtag2, "DeltaPhiLLept_mumu_afterbtag2",   datasetName,  "deltaPhi", "Entries",   10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_ee_afterbtag2,   "DeltaPhiLLept_ee_afterbtag2",     datasetName,  "deltaPhi", "Entries",   10, 0, 3.14);
	MyhistoManager.CreateHisto( DeltaPhiLLept_emu_afterbtag2,  "DeltaPhiLLept_emu_afterbtag2",    datasetName,  "deltaPhi", "Entries",   10, 0, 3.14);
	MyhistoManager.CreateHisto(tauPol_pion,"tauPol_pion",datasetName,"x", "Events" , 10,0,1);//addedBy_addHistToFWK.sh_130222_171848
	MyhistoManager.CreateHisto(tauPol_lep,"tauPol_lep",datasetName,"x", "Events" , 10,0,1);//addedBy_addHistToFWK.sh_130222_171953
	MyhistoManager.CreateHisto(tauPolReweighted_lep,"tauPolReweighted_lep",datasetName,"x", "Events" , 10,0,1);//addedBy_addHistToFWK.sh_130228_204836  
	MyhistoManager.CreateHisto(tauPolWeights_lep,"tauPolWeights_lep",datasetName,"weight", "Events" , 200,-2,2);
	MyhistoManager.CreateHisto(tauPolWeights_ee_afterbtag1,"tauPolWeights_ee_afterbtag1",datasetName,"weight", "Events" , 200,-2,2);
	MyhistoManager.CreateHisto(tauPolWeights_emu_afterbtag1,"tauPolWeights_emu_afterbtag1",datasetName,"weight", "Events" , 200,-2,2);
	MyhistoManager.CreateHisto(tauPolWeights_mumu_afterbtag1,"tauPolWeights_mumu_afterbtag1",datasetName,"weight", "Events" , 200,-2,2);

	MyhistoManager.CreateHisto( cosPhiLLept_mumu_afterbtag1, "cosPhiLLept_mumu_afterbtag1",   datasetName,  "cosPhi", "Entries",    10, -1, 1);
	MyhistoManager.CreateHisto( cosPhiLLept_ee_afterbtag1,   "cosPhiLLept_ee_afterbtag1",     datasetName,  "cosPhi", "Entries",    10, -1, 1);
	MyhistoManager.CreateHisto( cosPhiLLept_emu_afterbtag1,  "cosPhiLLept_emu_afterbtag1",    datasetName,  "cosPhi", "Entries",    10, -1, 1);

	MyhistoManager.CreateHisto( theO1_mumu_afterbtag1, "theO1_mumu_afterbtag1",	datasetName,  "theO1", "Entries",	10, -1, 1);
	MyhistoManager.CreateHisto( theO1_ee_afterbtag1,   "theO1_ee_afterbtag1",	datasetName,  "theO1", "Entries",	10, -1, 1);
	MyhistoManager.CreateHisto( theO1_emu_afterbtag1,  "theO1_emu_afterbtag1",	datasetName,  "theO1", "Entries",	10, -1, 1);





	MyhistoManager.CreateHisto( RecoTopMass_mumu_aftermetcut, "RecoTopMass_mumu_aftermetcut",   datasetName,  "deltaPhi", "Entries",   30, 100, 400);
	MyhistoManager.CreateHisto( RecoTopMass_ee_aftermetcut ,  "RecoTopMass_ee_aftermetcut",     datasetName,  "deltaPhi", "Entries",   30, 100, 400);
	MyhistoManager.CreateHisto( RecoTopMass_emu_aftermetcut,  "RecoTopMass_emu_aftermetcut",    datasetName,  "deltaPhi", "Entries",   30, 100, 400);

	MyhistoManager.CreateHisto( RecoTopMass_mumu_afterbtag1, "RecoTopMass_mumu_afterbtag1",   datasetName,  "deltaPhi", "Entries",    30, 100, 400);
	MyhistoManager.CreateHisto( RecoTopMass_ee_afterbtag1,   "RecoTopMass_ee_afterbtag1",     datasetName,  "deltaPhi", "Entries",    30, 100, 400);
	MyhistoManager.CreateHisto( RecoTopMass_emu_afterbtag1,  "RecoTopMass_emu_afterbtag1",    datasetName,  "deltaPhi", "Entries",    30, 100, 400);

	MyhistoManager.CreateHisto( RecoTopMass_mumu_afterbtag2, "RecoTopMass_mumu_afterbtag2",   datasetName,  "deltaPhi", "Entries",   30, 100, 400);
	MyhistoManager.CreateHisto( RecoTopMass_ee_afterbtag2,   "RecoTopMass_ee_afterbtag2",     datasetName,  "deltaPhi", "Entries",   30, 100, 400);
	MyhistoManager.CreateHisto( RecoTopMass_emu_afterbtag2,  "RecoTopMass_emu_afterbtag2",    datasetName,  "deltaPhi", "Entries",   30, 100, 400);



	MyhistoManager.CreateHisto( RecoMttbar_mumu_aftermetcut, "RecoMttbar_mumu_aftermetcut",   datasetName,  "RecoMttbar", "Entries",   100, 200, 1000);
	MyhistoManager.CreateHisto( RecoMttbar_ee_aftermetcut ,  "RecoMttbar_ee_aftermetcut",     datasetName,  "RecoMttbar", "Entries",   100, 200, 1000);
	MyhistoManager.CreateHisto( RecoMttbar_emu_aftermetcut,  "RecoMttbar_emu_aftermetcut",    datasetName,  "RecoMttbar", "Entries",   100, 200, 1000);

	MyhistoManager.CreateHisto( RecoMttbar_mumu_afterbtag1, "RecoMttbar_mumu_afterbtag1",   datasetName,  "RecoMttbar", "Entries",    100, 200, 1000);
	MyhistoManager.CreateHisto( RecoMttbar_ee_afterbtag1,   "RecoMttbar_ee_afterbtag1",     datasetName,  "RecoMttbar", "Entries",    100, 200, 1000);
	MyhistoManager.CreateHisto( RecoMttbar_emu_afterbtag1,  "RecoMttbar_emu_afterbtag1",    datasetName,  "RecoMttbar", "Entries",    100, 200, 1000);

	MyhistoManager.CreateHisto( RecoMttbar_mumu_afterbtag2, "RecoMttbar_mumu_afterbtag2",   datasetName,  "RecoMttbar", "Entries",   100, 200, 1000);
	MyhistoManager.CreateHisto( RecoMttbar_ee_afterbtag2,   "RecoMttbar_ee_afterbtag2",     datasetName,  "RecoMttbar", "Entries",   100, 200, 1000);
	MyhistoManager.CreateHisto( RecoMttbar_emu_afterbtag2,  "RecoMttbar_emu_afterbtag2",    datasetName,  "RecoMttbar", "Entries",   100, 200, 1000);



	MyhistoManager.CreateHisto(CSVDiscri_ee_aftermetcut,    "CSVDiscri_ee_aftermetcut",   datasetName, "CSV", "Entries", 20, 0, 1);
	MyhistoManager.CreateHisto(CSVDiscri_emu_aftermetcut,   "CSVDiscri_emu_aftermetcut",  datasetName, "CSV", "Entries", 20, 0, 1);
	MyhistoManager.CreateHisto(CSVDiscri_mumu_aftermetcut,  "CSVDiscri_mumu_aftermetcut", datasetName, "CSV", "Entries", 20, 0, 1);

	MyhistoManager.CreateHisto(CSVDiscriBjets_ee_aftermetcut,    "CSVDiscriBjets_ee_aftermetcut",   datasetName, "CSV", "Entries", 20, 0, 1);
	MyhistoManager.CreateHisto(CSVDiscriBjets_emu_aftermetcut,   "CSVDiscriBjets_emu_aftermetcut",  datasetName, "CSV", "Entries", 20, 0, 1);
	MyhistoManager.CreateHisto(CSVDiscriBjets_mumu_aftermetcut,  "CSVDiscriBjets_mumu_aftermetcut", datasetName, "CSV", "Entries", 20, 0, 1);


	MyhistoManager.CreateHisto2D(Reco_vs_gen_Mttbar_mumu_afterbtag1, "Reco_vs_gen_Mttbar_mumu_afterbtag1",   datasetName, "recomtt" ,  100, 0, 1000, "genmtt",  100, 0, 1000 );
	MyhistoManager.CreateHisto2D(Reco_vs_gen_Mttbar_ee_afterbtag1, "Reco_vs_gen_Mttbar_ee_afterbtag1",    datasetName, "recomtt", 100, 0, 1000,  "genmtt", 100,0, 1000 );
	MyhistoManager.CreateHisto2D(Reco_vs_gen_Mttbar_emu_afterbtag1, "Reco_vs_gen_Mttbar_emu_afterbtag1",    datasetName, "recomtt", 100, 0, 1000, "genmtt", 100, 0, 1000);


	MyhistoManager.CreateHisto(ttbarTransverseMass_mumu_afterbtag1, "ttbarTransverseMass_mumu_afterbtag1", datasetName, "mT", "Entries", 50, 0, 1500);
	MyhistoManager.CreateHisto(ttbarTransverseMass_ee_afterbtag1,   "ttbarTransverseMass_ee_afterbtag1",   datasetName, "mT", "Entries", 50, 0, 1500);
	MyhistoManager.CreateHisto(ttbarTransverseMass_emu_afterbtag1,  "ttbarTransverseMass_emu_afterbtag1",  datasetName, "mT", "Entries", 50, 0, 1500);




	MyhistoManager.CreateHisto(GEN_ttbarTransverseMass_mumu_afterbtag1, "GEN_ttbarTransverseMass_mumu_afterbtag1", datasetName, "mT", "Entries", 50, 0, 1500);
	MyhistoManager.CreateHisto(GEN_ttbarTransverseMass_ee_afterbtag1,   "GEN_ttbarTransverseMass_ee_afterbtag1",   datasetName, "mT", "Entries", 50, 0, 1500);
	MyhistoManager.CreateHisto(GEN_ttbarTransverseMass_emu_afterbtag1,  "GEN_ttbarTransverseMass_emu_afterbtag1",  datasetName, "mT", "Entries", 50, 0, 1500);

	MyhistoManager.CreateHisto(GEN_ttbarTransverseMass, "GEN_ttbarTransverseMass", datasetName, "mT", "Entries", 50, 0, 1500);

	MyhistoManager.CreateHisto2D(GEN_ttbarTransverseMass_vs_mttbar,    "GEN_ttbarTransverseMass_vs_mttbar",    datasetName,  "mttbar", 100, 0, 1500, "mT",100, 0, 1500);   

	MyhistoManager.CreateHisto2D(GENttbarMT_vs_RECOttbarMT_mumu_afterbtag1, "GENttbarMT_vs_RECOttbarMT_mumu_afterbtag1", datasetName, "mTGen", 50, 0, 1500, "mTReco", 50, 0, 1500);
	MyhistoManager.CreateHisto2D(GENttbarMT_vs_RECOttbarMT_ee_afterbtag1  , "GENttbarMT_vs_RECOttbarMT_ee_afterbtag1",   datasetName, "mTGen", 50, 0, 1500, "mTReco", 50, 0, 1500); 
	MyhistoManager.CreateHisto2D(GENttbarMT_vs_RECOttbarMT_emu_afterbtag1 , "GENttbarMT_vs_RECOttbarMT_emu_afterbtag1",  datasetName, "mTGen", 50, 0, 1500, "mTReco", 50, 0, 1500);
  /////////////////////////////////////
  //section for scripted added histos: createHisto statement
  ///////////////////////////////////////////////
  MyhistoManager.CreateHisto(ptWheightsUsed_mumu_after1btag,"ptWheightsUsed_mumu_after1btag", datasetName, "topPt", "ptWeights", 250,0,500);//addedBy_addHistToFWK.sh_130521_120454
  MyhistoManager.CreateHisto(ptWheightsUsed_emu_after1btag,"ptWheightsUsed_emu_after1btag", datasetName, "topPt", "ptWeights", 250,0,500);//addedBy_addHistToFWK.sh_130521_121514
  MyhistoManager.CreateHisto(ptWheightsUsed_ee_after1btag,"ptWheightsUsed_ee_after1btag", datasetName, "topPt", "ptWeights", 250,0,500);//addedBy_addHistToFWK.sh_130521_121654
  //nextHistWillBeAddedHereCreateWithHistoManager
  //end script added histo section
  ////////////////////////////////////////

	neventsel = 0;
	neventsel_recofailure = 0;
	neventsel_selfailure = 0;
	...
	if(doPDFreweight){
		for(unsigned int i=0; i<41; i++){
			...
			TString histoname = "DeltaPhiLLept_mumu_aftermetcut"+getIndexPDF(i);      
			TH1F histo1(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_mumu_aftermetcut_pdf.push_back(histo1);


			histoname = "DeltaPhiLLept_ee_aftermetcut"+getIndexPDF(i);      
			TH1F histo2(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_ee_aftermetcut_pdf.push_back(histo2);


			histoname = "DeltaPhiLLept_emu_aftermetcut"+getIndexPDF(i);      
			TH1F histo3(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_emu_aftermetcut_pdf.push_back(histo3);
			...
			histoname = "DeltaPhiLLept_mumu_afterbtag1"+getIndexPDF(i);      
			TH1F histo4(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_mumu_afterbtag1_pdf.push_back(histo4);


			histoname = "DeltaPhiLLept_ee_afterbtag1"+getIndexPDF(i);      
			TH1F histo5(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_ee_afterbtag1_pdf.push_back(histo5);


			histoname = "DeltaPhiLLept_emu_afterbtag1"+getIndexPDF(i);      
			TH1F histo6(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_emu_afterbtag1_pdf.push_back(histo6);
			...
			histoname = "DeltaPhiLLept_mumu_afterbtag2"+getIndexPDF(i);      
			TH1F histo7(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_mumu_afterbtag2_pdf.push_back(histo7);


			histoname = "DeltaPhiLLept_ee_afterbtag2"+getIndexPDF(i);      
			TH1F histo8(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_ee_afterbtag2_pdf.push_back(histo8);


			histoname = "DeltaPhiLLept_emu_afterbtag2"+getIndexPDF(i);      
			TH1F histo9(histoname.Data(), histoname.Data(),  10, 0, 3.14); 
			DeltaPhiLLept_emu_afterbtag2_pdf.push_back(histo9);
	...
	if (IReweight ) {
		...
		if(datasetName == "DYToLL_M10-50"||  isTTbarMCatNLO){

			cout << "init the right thing " << endl;
			//string datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/PUdata.root";
			string datafile;
			string mcfile;
			if(datasetName == "DYToLL_M10-50"){
				mcfile   = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/PU3DMC_Fall11.root"; 
				datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/dataPUhisto_2011AB_73.5mb_pixelLumi_diffBinning.root";

			}
			if(isTTbarMCatNLO                ){
				mcfile   = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/PU3DMC.root";
				//datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/dataPUhisto_2011AB_73.5mb_pixelLumi_diffBinning_bin25.root";
				datafile =

					//"/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/dataPUhisto_2011AB_73.5mb_pixelLumi_diffBinning_bin25_Up.root";
					"/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/dataPUhisto_2011AB_73.5mb_pixelLumi_diffBinning_bin25.root";
			}
		...
		if(datasetName != "DYToLL_M10-50" &&  !isTTbarMCatNLO){
			...
			if(!IReweight_puUp && !IReweight_puDown){
				//datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/default73.5mb.root";
				datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/dataPUhisto_2011AB_73.5mb_pixelLumi.root";
			}

			if( IReweight_puUp)   datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/default73.5mbUp.root";
			if( IReweight_puDown) datafile = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/default73.5mbDown.root";

			string mcfile   = "/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_06_19/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/PU3DMC.root";
			//cout << "loading  LumiWeights" <<  endl;
	...
	fOutput->Add(fHist);
	fOutput->Add(mcEventWeightHist);
	fOutput->Add(fFile);
	cout << "end SlaveBegin " << endl;
	if(doPDFreweight) LHAPDF::initPDFSet(lhapdfFile);
}
...
Bool_t ProofSelectorMyCutFlow::Process(Long64_t entry)
{
	...
	NTMET met			   = sel.GetMET(applyJES, scale, applyJER, ResFactor); 


	double MCsign = event->eventWeight/abs(event->eventWeight); 
	mcEventWeightHist->Fill(event->eventWeight);
	...
	/*if(isTTbarMCatNLO){
	  cout << "test ttbar info " << endl;
	  cout <<  " tmeme is " << event->TMEME << endl;
	  cout <<  "topAndDecays.size() " << event->topAndDecays.size() << endl;
	  cout <<  "wAndDecays.size() " << event->wAndDecays.size() << endl;
	  cout <<  "genLquarks.size() " <<  event->genLquarks.size()<< endl;

	  for(unsigned int i=0; i<event->wAndDecays.size(); i++){
	  cout << "wAndDecays[i].mcLepId " << event->wAndDecays[i].mcLepId << endl;
	  cout << "wAndDecays[i].Tmeme " << event->wAndDecays[i].Tmeme << endl;
	  cout << "wAndDecays[i].p4_Lep_gen.Pt " << event->wAndDecays[i].p4_Lep_gen.Pt() << endl;
	  cout << "wAndDecays[i].p4_Neu_gen " << event->wAndDecays[i].p4_Neu_gen.Pt() << endl;
	  cout << "wAndDecays[i].Q_Lep_gen " << event->wAndDecays[i].Q_Lep_gen << endl;

	  if (event->wAndDecays[i].p4_Lep_gen.Pt() > 0 ) cout << "found a lepton !!! " << endl;
	  }


	  }*/

	double mttbar_mt_gen = 0;
	double ttbarinvM = -1;
	deltaPhiGen = -1; deltaPhiRec = -1; totalEventWeight = -100; ttbarRec = -1; ttbarGen = -1;
         
        double globalTopPtWeight = -1;

	if(datasetName=="TTbar" || datasetName=="TTbarMCatNLO" ){

		if(event->TMEME==20 || event->TMEME==11010 || event->TMEME==22000 || 
				event->TMEME==2 || event->TMEME==10101 || event->TMEME==20200 || 
				event->TMEME==11 || event->TMEME==21100 || event->TMEME==11001 || event->TMEME==10110  
		  ){


			ttbarinvM = (event->topAndDecays[0].p4_t_gen+event->topAndDecays[1].p4_t_gen).M();
			ttbarGen = ttbarinvM;
			globalTopPtWeight = pow(topPtMcWeightFunc->Eval(event->topAndDecays[0].p4_t_gen.Pt())*topPtMcWeightFunc->Eval(event->topAndDecays[1].p4_t_gen.Pt()),0.5);//FH

			//TLorentzVector b1 = event->topAndDecays[0].p4_b_gen;
			//TLorentzVector b2 = event->topAndDecays[1].p4_b_gen;


			//TLorentzVector b1 = event->topAndDecays[0].p4_t_gen-event->wAndDecays[0].p4_W_gen;
			//TLorentzVector b2 = event->topAndDecays[1].p4_t_gen-event->wAndDecays[1].p4_W_gen;

			TLorentzVector b1 = event->topAndDecays[0].p4_t_gen - event->wAndDecays[0].p4_Lep_gen - event->wAndDecays[0].p4_Neu_gen ;
			TLorentzVector b2 = event->topAndDecays[1].p4_t_gen - event->wAndDecays[1].p4_Lep_gen - event->wAndDecays[1].p4_Neu_gen ;

			/*cout << "b1 mass " << b1.M() << endl;
			  cout << "b2 mass " << b2.M() << endl;

			  cout << " mass 1 " << (b1+event->wAndDecays[0].p4_W_gen).M() << endl;
			 */
			/*int sol1 = 1;
			  if( fabs((b1+event->wAndDecays[0].p4_W_gen).M()-172.5) > 0.5){

			  b1 = event->topAndDecays[0].p4_t_gen-event->wAndDecays[1].p4_W_gen;
			  b2 = event->topAndDecays[1].p4_t_gen-event->wAndDecays[0].p4_W_gen;
			  sol1 =2;
			  } */
			...
			/*cout << "b1 mass " << b1.M() << endl;
			  ...
			  cout << " mass 2 " << (b1+event->wAndDecays[1].p4_W_gen).M() << endl;
			  cout << "mass sol " << sol1 << " chosen " << endl;
			 */
			//cout << "event->topAndDecays[0].Q_gen  " << event->topAndDecays[0].Q_gen << endl;
			//cout << "event->wAndDecays[0].Q_Lep_gen  " << event->wAndDecays[0].Q_Lep_gen << endl;
			TLorentzVector lept1 = event->wAndDecays[0].p4_Lep_gen ;
			TLorentzVector lept2 = event->wAndDecays[1].p4_Lep_gen ;
			//      cout<<"testing all leps ";
			//      for(unsigned int testi = 0 ; testi < event->wAndDecays.size(); testi++){
			//       cout<<" , "<<event->wAndDecays[testi].p4_Lep_gen.Pz()<<" "<<event->wAndDecays[testi].p4_Lep_gen.Pt()<<" "<<event->wAndDecays[testi].mcLepId;
			//	}
			//      cout<<endl;
			TLorentzVector llbb_gen = b1+b2+lept1+lept2;

			mttbar_mt_gen = pow(
					llbb_gen.M()*llbb_gen.M()+
					2*(
						pow( llbb_gen.Pt()*llbb_gen.Pt() + llbb_gen.M()*llbb_gen.M(), 0.5)*llbb_gen.Pt()
						+llbb_gen.Pt()*llbb_gen.Pt()
					  )
					, 0.5);
			TLorentzVector ttbarsyst = (event->topAndDecays[0].p4_t_gen+event->topAndDecays[1].p4_t_gen);
			if(event->wAndDecays.size() > 5){
				TLorentzVector lept1_DeltaPhi = event->wAndDecays[event->wAndDecays.size()-2].p4_Lep_gen ;
				TLorentzVector lept2_DeltaPhi = event->wAndDecays[event->wAndDecays.size()-1].p4_Lep_gen ;
				//      cout<<"lept12 "<<lept1_DeltaPhi.Pt()<<" "<<lept2_DeltaPhi.Pt()<<" "<<event->wAndDecays.size()<<endl;

				if(lept1_DeltaPhi.Pt() > 0 && lept2_DeltaPhi.Pt() > 0)
					deltaPhiGen = fabs(lept1_DeltaPhi.DeltaPhi(lept2_DeltaPhi)); 
				if (deltaPhiGen < 0.0 || deltaPhiGen > 3.15){
					cout<<"strange deltaPhi "<<deltaPhiGen <<endl;
					deltaPhiGen = -2;
				}
			}
			else{ 
				cout<<"strange wAndDecaysSize "<<event->wAndDecays.size()<<endl;
				deltaPhiGen = -3;
			}
			totalEventWeight = MCsign * Luminosity*dataset->Xsection()/initMCevents[datasetName]; 
			TupleTreeBeforeSelection->Fill();
			/*cout << "---------------------------" << endl;
			  cout << "ttbarinvM     " << ttbarinvM << endl;
			  cout << "ttbar pt      " << ttbarsyst.Pt()  << endl;
			  cout << "mttbar_mt_gen " << mttbar_mt_gen  << endl;
			  cout << "llbb_gen.M()  " << llbb_gen.M() << endl;
			  cout << "llbb_gen.Pt() " << llbb_gen.Pt() << endl;
			  cout << "b1.Pt()    " << b1.Pt() <<  endl;
			  cout << "b2.Pt()    " << b2.Pt() <<  endl;
			  cout << "lept1.Pt() " << lept1.Pt() <<  endl;
			  cout << "lept2.Pt() " << lept2.Pt() <<  endl;*/
			/*if(llbb_gen.M() > ttbarinvM) cout << "found problemmsss !!! " << endl;
			  if(mttbar_mt_gen > ttbarinvM) cout << "mttbar_mt_gen sup " << endl;*/

			MyhistoManager.FillHisto(GEN_ttbarTransverseMass, "GEN_ttbarTransverseMass", mttbar_mt_gen, datasetName, true, 1);
			//MyhistoManager.FillHisto2D(GEN_ttbarTransverseMass_vs_mttbar, "GEN_ttbarTransverseMass_vs_mttbar", mttbar_mt_gen, ttbarinvM, datasetName, true, 1); 
			MyhistoManager.FillHisto2D(GEN_ttbarTransverseMass_vs_mttbar, "GEN_ttbarTransverseMass_vs_mttbar", ttbarinvM, mttbar_mt_gen, datasetName, true, 1); 
		}
	}
	...
	double therand = rand.Uniform(1);

	bool passrand = true;



	MyhistoManager.FillHisto(testRand_ee, "testRand_ee",therand , datasetName, true, 1);
	MyhistoManager.FillHisto(testRand_mumu, "testRand_mumu",therand , datasetName, true, 1);
	MyhistoManager.FillHisto(testRand_emu, "testRand_emu",therand , datasetName, true, 1);



	if(therand <= 0.5 && datasetName=="TTbar" && randsup > 0  ) passrand = false;
	if(therand > 0.5  && datasetName=="TTbar" && randsup < 0  ) passrand = false;

	if(!dorand) passrand = true;
	...
	if(passrand) {
		...
		//cout << "dataset->getNSkimmedEvent() " << dataset->getNSkimmedEvent() << endl;
		if(isTTbarMCatNLO)  weightITypeMC_save = Luminosity*dataset->Xsection()/dataset->getNSkimmedEvent();

		/*if(datasetName=="TTbarMCatNLO" ){
		  weightITypeMC_save = Luminosity*dataset->Xsection()/dataset->getNSkimmedEvent();

		  }*/


		double weightITypeMC=0;
		double weightTauPol = -1;
		double global_weightTauPol = 1;
		double lepEnergyRatioTau = -1; 
		//cout<<"start tau pol"<<endl;
		// tau polarization part
		if (event->wPlusAndDecays.size() > 0 && event->wPlusAndDecays[event->wPlusAndDecays.size()-1].mcLepId == -15 ){

			if (event->GeneratedtausLepPlusDecays.size() > 0 && event->GeneratedtausLepPlusDecays[0].size()  > 1 ){
				TLorentzVector boostedLep = getBoostedPionInWFrame(event->GeneratedtausLepPlusDecays[0][1],event->wPlusAndDecays[0].p4_W_gen) ;
				lepEnergyRatioTau = boostedLep.E()*2/event->wPlusAndDecays[0].p4_W_gen.M();
				MyhistoManager.FillHisto(tauPol_lep, "tauPol_lep",lepEnergyRatioTau , datasetName, true,  MCsign);//addedBy_addHistToFWK.sh_130222_171953
				if(tauPolReweighting){
					weightTauPol = tauPolReweightHistForMCatNLO->GetBinContent(tauPolReweightHistForMCatNLO->FindBin(lepEnergyRatioTau));
					std::cout<< "weightTauPol "<<weightTauPol<<std::endl;
					MyhistoManager.FillHisto(tauPolReweighted_lep,"tauPolReweighted_lep",lepEnergyRatioTau, datasetName, true,weightTauPol* MCsign);//addedBy_addHistToFWK.sh_130228_204836
					//weightITypeMC_save = weightITypeMC_save*weightTauPol;
					global_weightTauPol *= weightTauPol;
					MyhistoManager.FillHisto(tauPolWeights_lep,"tauPolWeights_lep",weightTauPol, datasetName, true,1);
				}
			}
			if (event->GeneratedtausPionPlusDecays.size() > 0 && event->GeneratedtausPionPlusDecays[0].size() > 1){ 
				TLorentzVector boostedPion = getBoostedPionInWFrame(event->GeneratedtausPionPlusDecays[0][1],event->wPlusAndDecays[0].p4_W_gen) ;
				MyhistoManager.FillHisto(tauPol_pion,"tauPol_pion",boostedPion.E()*2/event->wPlusAndDecays[0].p4_W_gen.M(),datasetName,true,  MCsign);//addedBy_addHistToFWK.sh_130222_171848
			}

			//or ( len(test.GeneratedtausLepMinusDecays) != 0 and len(test.GeneratedtausLepMinusDecays[0]) > 1 )or (len(test.GeneratedtausPionMinusDecays) != 0 and len(test.GeneratedtausPionMinusDecays[0]) > 1):

		}


		if (event->wMinusAndDecays.size() > 0 && event->wMinusAndDecays[event->wMinusAndDecays.size()-1].mcLepId == 15 ){
			if (event->GeneratedtausLepMinusDecays.size() > 0 && event->GeneratedtausLepMinusDecays[0].size()  > 1 ){
				TLorentzVector boostedLep = getBoostedPionInWFrame(event->GeneratedtausLepMinusDecays[0][1],event->wMinusAndDecays[0].p4_W_gen) ;
				MyhistoManager.FillHisto(tauPol_lep, "tauPol_lep",boostedLep.E()*2/event->wMinusAndDecays[0].p4_W_gen.M() , datasetName, true,  MCsign);//addedBy_addHistToFWK.sh_130222_171953
				if(tauPolReweighting){          
					weightTauPol = tauPolReweightHistForMCatNLO->GetBinContent(tauPolReweightHistForMCatNLO->FindBin(boostedLep.E()*2/event->wMinusAndDecays[0].p4_W_gen.M()));
					MyhistoManager.FillHisto(tauPolReweighted_lep,"tauPolReweighted_lep",boostedLep.E()*2/event->wPlusAndDecays[0].p4_W_gen.M(), datasetName, true,weightTauPol* MCsign);//addedBy_addHistToFWK.sh_130228_204836
					global_weightTauPol *= weightTauPol;
					MyhistoManager.FillHisto(tauPolWeights_lep,"tauPolWeights_lep",weightTauPol, datasetName, true,1);
					//weightITypeMC_save = weightITypeMC_save*weightTauPol;
				}
			}
			if (event->GeneratedtausPionMinusDecays.size() > 0 && event->GeneratedtausPionMinusDecays[0].size() > 1){
				TLorentzVector boostedPion = getBoostedPionInWFrame(event->GeneratedtausPionMinusDecays[0][1],event->wMinusAndDecays[0].p4_W_gen) ;
				MyhistoManager.FillHisto(tauPol_pion,"tauPol_pion",boostedPion.E()*2/event->wMinusAndDecays[0].p4_W_gen.M(),datasetName,true,  MCsign);//addedBy_addHistToFWK.sh_130222_171848
			}
			//std::vector<TH1F> tauPol_pion;//addedBy_addHistToFWK.sh_130222_171848
			//std::vector<TH1F> tauPol_lep;//addedBy_addHistToFWK.sh_130222_171953
			//std::vector<TH1F> tauPolReweighted_lep;//addedBy_addHistToFWK.sh_130228_204836
		}
		//cout<<"end tau pol"<<endl;
		// end tau polarization part
		//cout << "event->eventWeight " << event->eventWeight << endl;

		if(isTTbarMCatNLO && event->eventWeight < 0) weightITypeMC_save = -1*weightITypeMC_save;
		...
		for (int IChannel=0; IChannel<3; IChannel++) {
			...
			if ( datasetName!="DataEG" && datasetName!="DataMu" && 
				...
				if(IReweight ){
					...
					if(datasetName == "DYToLL_M10-50" || isTTbarMCatNLO)  {
						...
						if(datasetName == "DYToLL_M10-50") weightITypeMC = weightITypeMC_save*LumiWeights->ITweight3BX(ave_npu);
						else weightITypeMC = weightITypeMC_save*LumiWeights->ITweight(event->num_pileup_bc0);



						/*cout << "LumiWeights->ITweight3BX(ave_npu) " << LumiWeights->ITweight3BX(ave_npu) << endl;
						  cout << "LumiWeights->ITweight3BX(ave_npu) " << LumiWeights->ITweight(ave_npu) << endl;
			...
			//if(datasetName=="TTbarMCatNLO") {isTTbarMCatNLO	= true; datasetName=="TTbar";}
			...
			if ( datasetName=="TTbar" || 
					...
					datasetName=="TTbarMCatNLO"
						...
						if(isTTbarMCatNLO) WeightForBranchingRatio = 1;
						/*if(){
			...
			/*if(isTTbarMCatNLO){
			  vector<TopTree::NTElectron> candElec_tmp;
			  vector<TopTree::NTMuon> candMuon_tmp;
			  string CandType0_tmp = "";
			  sel.GetLeptonPair(candMuon, candElec, CandType0_tmp ); 
			  IsSignal = false;
			  if(IChannel==0 && CandType0_tmp == "mumu")  IsSignal = true;
			  if(IChannel==1 && CandType0_tmp == "ee")  IsSignal = true;
			  if(IChannel==2 && CandType0_tmp == "emu")  IsSignal = true;

			  }*/


			//if(IsSignal) cout << "is data true " << endl;
			...
			for(unsigned int imuon=0; imuon<allMuons.size(); imuon++){
				...
				MyhistoManager.FillHisto(LeptonIso_e, "LeptonIso_e", isoRel, datasetName, IsSignal, 1);
			...
			for(unsigned int ielec=0; ielec<allElectrons.size(); ielec++){
				...
				MyhistoManager.FillHisto(LeptonIso_mu, "LeptonIso_mu", isoRel, datasetName, IsSignal, 1);
			...
			//cout << " Dweight[ITypeMC]  " <<  Dweight[ITypeMC] << "  tmeme " << event->TMEME << endl;
			...
			if(producePLRPlots) plotsFurnisher.LoadEvent (sel, ChannelName, IsSignal, ITypeMC, event->TMEME, Dweight[ITypeMC], 
					*dataset, SF_trig_mu*vSF_met_mumu[0], SF_trig_emu*vSF_met_emu[0], SF_trig_ee*vSF_met_ee[0], SF_e,SF_mu, SF_DY_ee, SF_DY_mm,
			...
			//if(IsSignal && IChannel == 0) cout << "is a mumu event " << endl;
			//if(IsSignal && IChannel == 1) cout << "is a ee event " << endl;
			//if(IsSignal && IChannel == 2) cout << "is a emu event " << endl;

			//if( IChannel == 0) cout << event->TMEME  << endl;
			...
			if (  sel.passTriggerSelection ( dataset, ChannelName)     ) {
				...
				//if(IsSignal && IChannel == 0) cout << "   is a mumu event which pass trigger " << endl;
				//if(IsSignal && IChannel == 1) cout << "   is a ee event which pass trigger " << endl;
				//if(IsSignal && IChannel == 2) cout << "   is a emu event which pass trigger " << endl;
				...
				if(datasetName=="DataEG" || datasetName=="DataMu"  
					...
					//cout << "candMuon.size() " << candMuon.size()  << endl;
					//cout << "candElec.size() " << candElec.size()  << endl;
				}
				...
				if(ApplyLeptonSF){
					...
					if (CandType=="mumu" &&  !IsData && (  datasetName != "Wjets" &&  datasetName != "QCD"  
								&& !( (datasetName  =="TTbar" || datasetName  =="TTbarMCatNLO") &&  !IsSignal ) )   ) {
				...
				if ( ((CandType=="mumu" && ChannelName=="mumu")   || 
					...
					//DYEstimation
					if (ChannelName=="emu" && sel.GetSelectedJets(candMuon, candElec, applyJES, scale, applyJER, ResFactor).size()>=2 ) {
					...
					vector<NTJet>      theselJets = sel.GetSelectedJets(candMuon, candElec, applyJES, scale, applyJER, ResFactor);
						...
						if ( theselJets.size()>=2 )  {
							...
							if ( ((ChannelName=="mumu" || ChannelName=="ee") &&  met.p4.Et()>sel.GetMETCut().second) ||
								...
								if( datasetName=="TTbar" && IsSignal ){
									...
									if (ChannelName=="mumu"){ 
										Dweight[ITypeMC]*= vSF_met_mumu[0];
										EventYieldWeightError = Dweight[ITypeMC]*Dweight[ITypeMC] ;
									}
									if (ChannelName=="emu"){ 
										Dweight[ITypeMC]*= vSF_met_emu[0];
										EventYieldWeightError =  Dweight[ITypeMC]*Dweight[ITypeMC]  ;
									}
									if (ChannelName=="ee"){ 
										Dweight[ITypeMC]*= vSF_met_ee[0];
										EventYieldWeightError =  Dweight[ITypeMC]*Dweight[ITypeMC] ;
									}
								...
								for(unsigned int j=0;j<theselJets.size();j++){
									double csvDiscri = theselJets[j].GetDiscri(string("combinedSecondaryVertexBJetTags")) ;
									if(CandType=="mumu" && ChannelName=="mumu"){
										MyhistoManager.FillHisto(CSVDiscri_mumu_aftermetcut, "CSVDiscri_mumu_aftermetcut", csvDiscri, datasetName, IsSignal, Dweight[ITypeMC]);
										if( abs(theselJets[j].partonFlavour) == 5) MyhistoManager.FillHisto(CSVDiscriBjets_mumu_aftermetcut, "CSVDiscriBjets_mumu_aftermetcut", csvDiscri, datasetName, IsSignal, Dweight[ITypeMC]);		 
									}
									if(CandType=="emu" && ChannelName=="emu"){
										MyhistoManager.FillHisto(CSVDiscri_emu_aftermetcut, "CSVDiscri_emu_aftermetcut", csvDiscri, datasetName, IsSignal, Dweight[ITypeMC]);
										if( abs(theselJets[j].partonFlavour) == 5) MyhistoManager.FillHisto(CSVDiscriBjets_emu_aftermetcut, "CSVDiscriBjets_emu_aftermetcut", csvDiscri, datasetName, IsSignal, Dweight[ITypeMC]);		 
									}
									if(CandType=="ee" && ChannelName=="ee"){
										MyhistoManager.FillHisto(CSVDiscri_ee_aftermetcut, "CSVDiscri_ee_aftermetcut", csvDiscri, datasetName, IsSignal, Dweight[ITypeMC]);
										if( abs(theselJets[j].partonFlavour) == 5) MyhistoManager.FillHisto(CSVDiscriBjets_ee_aftermetcut, "CSVDiscriBjets_ee_aftermetcut", csvDiscri, datasetName, IsSignal, Dweight[ITypeMC]);		 
									}
								...
								double weight_SFmet_1btag[3] = {1,1,1};
								double weight_SFmet_2btag[3] = {1,1,1};
								...
								if( datasetName=="TTbar" && IsSignal ){
									...
									if (ChannelName=="mumu"){ 
										weight_SFmet_1btag[0]= vSF_met_mumu[1];
										EventYieldWeightError_Sup1btag += vSF_met_mumu_error[1]*Dweight[ITypeMC]*vSF_met_mumu_error[1]*Dweight[ITypeMC];
									}
									if (ChannelName=="emu"){ 
										weight_SFmet_1btag[1]= vSF_met_emu[1];
										EventYieldWeightError_Sup1btag +=  vSF_met_emu_error[1]*Dweight[ITypeMC]*vSF_met_emu_error[1]*Dweight[ITypeMC];
									}
									if (ChannelName=="ee"){ 
										weight_SFmet_1btag[2]= vSF_met_ee[1];
										EventYieldWeightError_Sup1btag +=  vSF_met_ee_error[1]*Dweight[ITypeMC]*vSF_met_ee_error[1]*Dweight[ITypeMC];
									}

									if (ChannelName=="mumu"){ 
										weight_SFmet_2btag[0]= vSF_met_mumu[2];
										EventYieldWeightError_Sup2btag += vSF_met_mumu_error[2]*Dweight[ITypeMC]*vSF_met_mumu_error[2]*Dweight[ITypeMC];
									}
									if (ChannelName=="emu"){ 
										weight_SFmet_2btag[1]= vSF_met_emu[2];
										EventYieldWeightError_Sup2btag +=  vSF_met_emu_error[2]*Dweight[ITypeMC]*vSF_met_emu_error[2]*Dweight[ITypeMC];
									}
									if (ChannelName=="ee"){ 
										weight_SFmet_2btag[2]= vSF_met_ee[2];
										EventYieldWeightError_Sup2btag += vSF_met_ee_error[2]*Dweight[ITypeMC]*vSF_met_ee_error[2]*Dweight[ITypeMC] ;
									}

								}
								weight_SFtotal_1btag[0] =  weight_SFDY_1btag[0] * weight_SFfakeBck_1btag[0] * weight_SFmet_1btag[0];
								weight_SFtotal_1btag[1] =  weight_SFDY_1btag[1] * weight_SFfakeBck_1btag[1] * weight_SFmet_1btag[1];
								weight_SFtotal_1btag[2] =  weight_SFDY_1btag[2] * weight_SFfakeBck_1btag[2] * weight_SFmet_1btag[2];


								weight_SFtotal_2btag[0] =  weight_SFDY_2btag[0] * weight_SFfakeBck_2btag[0] * weight_SFmet_2btag[0];
								weight_SFtotal_2btag[1] =  weight_SFDY_2btag[1] * weight_SFfakeBck_2btag[1] * weight_SFmet_2btag[1];
								weight_SFtotal_2btag[2] =  weight_SFDY_2btag[2] * weight_SFfakeBck_2btag[2] * weight_SFmet_2btag[2];
								...
								if(doPDFreweight)pdfWeights= getPDFweights(event->x1, event->x2, event->Q_scale, event->partonFlavor1, event->partonFlavor2);


								//calcul deltaPhi for spin correlations
								double deltaPhi = -1;
								if(CandType=="mumu" && ChannelName=="mumu") deltaPhi = candMuon[0].p4.DeltaPhi(candMuon[1].p4);
								if(CandType=="ee"   && ChannelName=="ee"  ) deltaPhi = candElec[0].p4.DeltaPhi(candElec[1].p4);
								if(CandType=="emu"  && ChannelName=="emu" ) deltaPhi = candMuon[0].p4.DeltaPhi(candElec[0].p4);

								deltaPhi = fabs(deltaPhi);

								//if(deltaPhi < 0 ) cout << "deltaphi neg " << endl;
								...
								if(CandType=="mumu" && ChannelName=="mumu"){ 
									...
									//calcul deltaPhi for spin correlations
									MyhistoManager.FillHisto(DeltaPhiLLept_mumu_aftermetcut, "DeltaPhiLLept_mumu_aftermetcut", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);


									if(doPDFreweight) fillPDFHisto(deltaPhi, datasetName, IsSignal, Dweight[ITypeMC], pdfWeights, "DeltaPhiLLept_mumu_aftermetcut", &DeltaPhiLLept_mumu_aftermetcut_pdf);  
								...
								if(CandType=="ee" && ChannelName=="ee"){
									...
									MyhistoManager.FillHisto(DeltaPhiLLept_ee_aftermetcut, "DeltaPhiLLept_ee_aftermetcut", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
									if(doPDFreweight) fillPDFHisto(deltaPhi, datasetName, IsSignal, Dweight[ITypeMC], pdfWeights, "DeltaPhiLLept_ee_aftermetcut", &DeltaPhiLLept_ee_aftermetcut_pdf); 
								...
								if(CandType=="emu" && ChannelName=="emu"){
									...
									MyhistoManager.FillHisto(DeltaPhiLLept_emu_aftermetcut, "DeltaPhiLLept_emu_aftermetcut", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
									if(doPDFreweight) fillPDFHisto(deltaPhi, datasetName, IsSignal, Dweight[ITypeMC], pdfWeights, "DeltaPhiLLept_emu_aftermetcut", &DeltaPhiLLept_emu_aftermetcut_pdf); 
								...
								if(doTopReco){
									TopMassVariables * topMassVariables = new TopMassVariables(theselJets, candElec, candMuon, met, CandType, sel);

									TopMassVariables * tmv;
									if( (CandType=="ee"    && ChannelName=="ee"  )  ||
											(CandType=="emu"   && ChannelName=="emu" )  ||
											(CandType=="mumu"  && ChannelName=="mumu")   ) tmv = amwt->findMass( *topMassVariables );


									topmass	= tmv->peakMass();
									ttbarmass = (tmv->getTop1P4() + tmv->getTop2P4()).M();
									//cout << "ttbarmass reco " << ttbarmass << endl;
									//cout << "ttbarmass gen  " << ttbarinvM << endl;
									//cout << "topmass " <<  topmass<< endl;
									//double mttbar = 


									TLorentzVector thetop1 = tmv->getTop1P4();
									TLorentzVector thetop2 = tmv->getTop2P4();

									TLorentzVector posLept_Boost = tmv->posLeptonLV();
									TLorentzVector negLept_Boost = tmv->negLeptonLV();


									//top1 with positive lepton
									posLept_Boost.Boost( TVector3(-1.0*thetop1.BoostVector().Px(),-1.0*thetop1.BoostVector().Py(),-1.0*thetop1.BoostVector().Pz()));
									//top2 with negative lepton
									negLept_Boost.Boost( TVector3(-1.0*thetop2.BoostVector().Px(),-1.0*thetop2.BoostVector().Py(),-1.0*thetop2.BoostVector().Pz()));


									cosPhi = cos( posLept_Boost.Vect().Angle(negLept_Boost.Vect()));

									TVector3 topdirection = thetop1.BoostVector().Unit();


									theO1 = (posLept_Boost.Vect().Unit()).Cross(negLept_Boost.Vect().Unit()).Dot(topdirection);
									...
									if( tmv->peakWeight() <  5 ) {
										...
										ttbarmass= -1;
										cosPhi=-100;
										//cout << "fail weight cuts " << endl;
									}

									neventsel++;
									if( tmv->peakWeight() <= 0.0000001)  neventsel_recofailure++;
									if( tmv->peakWeight() <  5 && tmv->peakWeight() >= 0 )  neventsel_selfailure++;
								}


								if(CandType=="ee"   && ChannelName=="ee"    ) MyhistoManager.FillHisto(RecoTopMass_ee_aftermetcut,   "RecoTopMass_ee_aftermetcut",   topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
								if(CandType=="emu"  && ChannelName=="emu"   ) MyhistoManager.FillHisto(RecoTopMass_emu_aftermetcut,  "RecoTopMass_emu_aftermetcut",  topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
								if(CandType=="mumu" && ChannelName=="mumu"  ) MyhistoManager.FillHisto(RecoTopMass_mumu_aftermetcut, "RecoTopMass_mumu_aftermetcut", topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
								...
								if(CandType=="ee"   && ChannelName=="ee"    ){
									...
									llbb = candElec[0].p4+candElec[1].p4 ;

								}
								if(CandType=="emu"  && ChannelName=="emu"   ){
									llbb = candElec[0].p4+candMuon[0].p4 ;

								} 
								if(CandType=="mumu" && ChannelName=="mumu"  ){
									llbb = candMuon[0].p4+candMuon[1].p4 ;

								}

								llbb +=  theselJets[0].p4+theselJets[1].p4;
								mttbar_mt = pow(
										llbb.M()*llbb.M()+
										2*(
											pow( llbb.Pt()*llbb.Pt() + llbb.M()*llbb.M(), 0.5)*llbb.Pt()
											+llbb.Pt()*llbb.Pt()
										  )
										, 0.5);
								// mttbar_mt sel
								if(mttbar_mt > -10000){	 
									...
									if(!IsData && ( datasetName == "TTbar" ||  datasetName == "TTbarMCatNLO") ){
										if(CandType=="ee"   && ChannelName=="ee"    ){
											MyhistoManager.FillHisto(GEN_ttbarTransverseMass_ee_afterbtag1 ,   "GEN_ttbarTransverseMass_ee_afterbtag1",   mttbar_mt_gen,  datasetName, IsSignal, Dweight[ITypeMC]);
											if(NBtaggedJets>=1) MyhistoManager.FillHisto2D(GENttbarMT_vs_RECOttbarMT_ee_afterbtag1, "GENttbarMT_vs_RECOttbarMT_ee_afterbtag1",mttbar_mt_gen ,mttbar_mt , datasetName, IsSignal, Dweight[ITypeMC]); 
										}
										if(CandType=="emu"  && ChannelName=="emu"   ){
											MyhistoManager.FillHisto(GEN_ttbarTransverseMass_emu_afterbtag1 ,  "GEN_ttbarTransverseMass_emu_afterbtag1",  mttbar_mt_gen,  datasetName, IsSignal, Dweight[ITypeMC]);
											if(NBtaggedJets>=1) MyhistoManager.FillHisto2D(GENttbarMT_vs_RECOttbarMT_emu_afterbtag1, "GENttbarMT_vs_RECOttbarMT_emu_afterbtag1",mttbar_mt_gen ,mttbar_mt , datasetName, IsSignal, Dweight[ITypeMC]);
										}
										if(CandType=="mumu" && ChannelName=="mumu"  ){
											MyhistoManager.FillHisto(GEN_ttbarTransverseMass_mumu_afterbtag1 , "GEN_ttbarTransverseMass_mumu_afterbtag1", mttbar_mt_gen,  datasetName, IsSignal, Dweight[ITypeMC]);
											if(NBtaggedJets>=1) MyhistoManager.FillHisto2D(GENttbarMT_vs_RECOttbarMT_mumu_afterbtag1, "GENttbarMT_vs_RECOttbarMT_mumu_afterbtag1",mttbar_mt_gen ,mttbar_mt , datasetName, IsSignal, Dweight[ITypeMC]);
										}
									...
									if (sel.GetFlagb() == 0 || IsData) {
										if ( NBtaggedJets>=1 )  {
											...
											if(CandType=="ee"   && ChannelName=="ee"  ) {
												...
												MyhistoManager.FillHisto(DeltaPhiLLept_ee_afterbtag1, "DeltaPhiLLept_ee_afterbtag1", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
                                                                                                MyhistoManager.FillHisto(DeltaPhiLLept_topPtReweighted_ee_afterbtag1,"DeltaPhiLLept_topPtReweighted_ee_afterbtag1",deltaPhi,  datasetName, IsSignal,Dweight[ITypeMC]*global_weightTauPol*globalTopPtWeight);
                                                                                                MyhistoManager.FillHisto(ptWheightsUsed_ee_after1btag, "ptWheightsUsed_ee_after1btag", globalTopPtWeight, datasetName, IsSignal, Dweight[ITypeMC]*global_weightTauPol);//addedBy_addHistToFWK.sh_130521_121654
												MyhistoManager.FillHisto(DeltaPhiLLept_ee_afterbtag1_tauPolReweighted, "DeltaPhiLLept_ee_afterbtag1_tauPolReweighted", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(tauPolWeights_ee_afterbtag1,"tauPolWeights_ee_afterbtag1",global_weightTauPol,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(cosPhiLLept_ee_afterbtag1,   "cosPhiLLept_ee_afterbtag1",     cosPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(theO1_ee_afterbtag1,   "theO1_ee_afterbtag1",     theO1,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(RecoTopMass_ee_afterbtag1,   "RecoTopMass_ee_afterbtag1",   topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												if(ttbarinvM > 0) MyhistoManager.FillHisto(RecoMttbar_ee_afterbtag1,  "RecoMttbar_ee_afterbtag1",  ttbarmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto2D(Reco_vs_gen_Mttbar_ee_afterbtag1,  "Reco_vs_gen_Mttbar_ee_afterbtag1",  ttbarmass, ttbarinvM , datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(HNjets_ee_aftermetbtag1,   "Njets_ee_aftermetbtag1",    JetMul ,datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(ttbarTransverseMass_ee_afterbtag1, "ttbarTransverseMass_ee_afterbtag1", mttbar_mt, datasetName, IsSignal, Dweight[ITypeMC]);
											}
											else if(CandType=="emu"  && ChannelName=="emu" )  {
												MyhistoManager.FillHisto(HNjets_emu_aftermetbtag1,  "Njets_emu_aftermetbtag1",   JetMul ,datasetName, IsSignal, Dweight[ITypeMC]);                  
												MyhistoManager.FillHisto(HHT_emu_aftermetbtag1,  "HHT_emu_aftermetbtag1",  HT,datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(HInvM_emu_pair_aftermetbtag1, "InvemuMassPair_aftermetbtag1" ,InvDilMass,datasetName   ,
														IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(DeltaPhiLLept_emu_afterbtag1, "DeltaPhiLLept_emu_afterbtag1", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(DeltaPhiLLept_emu_afterbtag1_tauPolReweighted, "DeltaPhiLLept_emu_afterbtag1_tauPolReweighted", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]*global_weightTauPol);
                                                                                                MyhistoManager.FillHisto(DeltaPhiLLept_topPtReweighted_emu_afterbtag1,"DeltaPhiLLept_topPtReweighted_emu_afterbtag1,",deltaPhi,datasetName,IsSignal,Dweight[ITypeMC]*global_weightTauPol*globalTopPtWeight);
                                                                                                MyhistoManager.FillHisto(ptWheightsUsed_emu_after1btag, "ptWheightsUsed_emu_after1btag", globalTopPtWeight, datasetName, IsSignal, Dweight[ITypeMC]*global_weightTauPol);//addedBy_addHistToFWK.sh_130521_121514
												MyhistoManager.FillHisto(tauPolWeights_emu_afterbtag1,"tauPolWeights_emu_afterbtag1",global_weightTauPol,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(cosPhiLLept_emu_afterbtag1,   "cosPhiLLept_emu_afterbtag1",   cosPhi,    datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(theO1_emu_afterbtag1,   "theO1_emu_afterbtag1",   theO1,    datasetName, IsSignal, Dweight[ITypeMC]);
												if( ttbarmass> 0) MyhistoManager.FillHisto2D(Reco_vs_gen_Mttbar_emu_afterbtag1,  "Reco_vs_gen_Mttbar_emu_afterbtag1", ttbarmass  , ttbarinvM,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(RecoTopMass_emu_afterbtag1,  "RecoTopMass_emu_afterbtag1",  topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(RecoMttbar_emu_afterbtag1,  "RecoMttbar_emu_afterbtag1",  ttbarmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(ttbarTransverseMass_emu_afterbtag1, "ttbarTransverseMass_emu_afterbtag1", mttbar_mt, datasetName, IsSignal, Dweight[ITypeMC]);
											}
											else if(CandType=="mumu" && ChannelName=="mumu") { 
												MyhistoManager.FillHisto(HNjets_mumu_aftermetbtag1, "Njets_mumu_aftermetbtag1",  JetMul ,datasetName, IsSignal, Dweight[ITypeMC]);                 
												...
												MyhistoManager.FillHisto(DeltaPhiLLept_mumu_afterbtag1, "DeltaPhiLLept_mumu_afterbtag1", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
                                                                                                MyhistoManager.FillHisto(DeltaPhiLLept_topPtReweighted_mumu_afterbtag1,"DeltaPhiLLept_topPtReweighted_mumu_afterbtag1", deltaPhi,  datasetName, IsSignal,Dweight[ITypeMC]*global_weightTauPol*globalTopPtWeight);
                                                                                                MyhistoManager.FillHisto(ptWheightsUsed_mumu_after1btag, "ptWheightsUsed_mumu_after1btag", globalTopPtWeight, datasetName, IsSignal, Dweight[ITypeMC]*global_weightTauPol);//addedBy_addHistToFWK.sh_130521_120454
												MyhistoManager.FillHisto(DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted, "DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]*global_weightTauPol);
												MyhistoManager.FillHisto(tauPolWeights_mumu_afterbtag1,"tauPolWeights_mumu_afterbtag1",global_weightTauPol,  datasetName, IsSignal, Dweight[ITypeMC]);

												MyhistoManager.FillHisto(cosPhiLLept_mumu_afterbtag1,   "cosPhiLLept_mumu_afterbtag1",   cosPhi,    datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(theO1_mumu_afterbtag1,   "theO1_mumu_afterbtag1",   theO1,    datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(RecoTopMass_mumu_afterbtag1,  "RecoTopMass_mumu_afterbtag1",  topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												if(ttbarmass > 0) MyhistoManager.FillHisto2D(Reco_vs_gen_Mttbar_mumu_afterbtag1,  "Reco_vs_gen_Mttbar_mumu_afterbtag1", ttbarmass , ttbarinvM,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(RecoMttbar_mumu_afterbtag1,  "RecoMttbar_mumu_afterbtag1",  ttbarmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(ttbarTransverseMass_mumu_afterbtag1, "ttbarTransverseMass_mumu_afterbtag1", mttbar_mt, datasetName, IsSignal, Dweight[ITypeMC]);
											}
										}
										if ( NBtaggedJets>=2 )  {
											...
											if(CandType=="ee"   && ChannelName=="ee"  ) {
												...
												MyhistoManager.FillHisto(DeltaPhiLLept_ee_afterbtag2, "DeltaPhiLLept_ee_afterbtag2", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(RecoTopMass_ee_afterbtag2,   "RecoTopMass_ee_afterbtag2",   topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
											}
											if(CandType=="emu"  && ChannelName=="emu" ) {
												...
												MyhistoManager.FillHisto(RecoTopMass_emu_afterbtag2,   "RecoTopMass_emu_afterbtag2",   topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(DeltaPhiLLept_emu_afterbtag2, "DeltaPhiLLept_emu_afterbtag2", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
											}
											if(CandType=="mumu" && ChannelName=="mumu") {
												...
												MyhistoManager.FillHisto(RecoTopMass_mumu_afterbtag2,   "RecoTopMass_mumu_afterbtag2",   topmass,  datasetName, IsSignal, Dweight[ITypeMC]);
												MyhistoManager.FillHisto(DeltaPhiLLept_mumu_afterbtag2, "DeltaPhiLLept_mumu_afterbtag2", deltaPhi,  datasetName, IsSignal, Dweight[ITypeMC]);
											}
										}
									}
									else if (sel.GetFlagb() == 1) {
										...
										// fill once Tree
										if( ((CandType=="ee"   && ChannelName=="ee" ) || CandType=="emu"  && ChannelName=="emu" || CandType=="mumu"  && ChannelName=="mumu" ) && IsSignal && TreeNotAlreadyFilled ){
											deltaPhiRec = deltaPhi;
											totalEventWeight = (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2];			
											ttbarRec = ttbarmass; 
											ttbarGen = ttbarinvM;
											TupleTreeUnfolding->Fill();
											TreeNotAlreadyFilled = false;
										}	
										if(CandType=="ee"   && ChannelName=="ee"  ) {
											...
											MyhistoManager.FillHisto(DeltaPhiLLept_ee_afterbtag1, "DeltaPhiLLept_ee_afterbtag1", deltaPhi,  datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
                                                                                        MyhistoManager.FillHisto(DeltaPhiLLept_topPtReweighted_ee_afterbtag1, "DeltaPhiLLept_topPtReweighted_ee_afterbtag1", deltaPhi,  datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]*global_weightTauPol*globalTopPtWeight);
                                                                                        MyhistoManager.FillHisto(ptWheightsUsed_ee_after1btag, "ptWheightsUsed_ee_after1btag", globalTopPtWeight, datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]*global_weightTauPol);//addedBy_addHistToFWK.sh_130521_121654
											MyhistoManager.FillHisto(DeltaPhiLLept_ee_afterbtag1_tauPolReweighted, "DeltaPhiLLept_ee_afterbtag1_tauPolReweighted", deltaPhi,  datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]*global_weightTauPol);
											MyhistoManager.FillHisto(tauPolWeights_ee_afterbtag1,"tauPolWeights_ee_afterbtag1",global_weightTauPol,  datasetName, IsSignal, Dweight[ITypeMC]);

											MyhistoManager.FillHisto(cosPhiLLept_ee_afterbtag1,   "cosPhiLLept_ee_afterbtag1",   cosPhi,    datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
											MyhistoManager.FillHisto(theO1_ee_afterbtag1,   "theO1_ee_afterbtag1",   theO1,    datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
											//if ( NBtaggedJets>=1 ){
											MyhistoManager.FillHisto(RecoTopMass_ee_afterbtag1, "RecoTopMass_ee_afterbtag1", topmass,  datasetName, IsSignal,(1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2] );
											MyhistoManager.FillHisto(RecoMttbar_ee_afterbtag1, "RecoMttbar_ee_afterbtag1", ttbarmass,  datasetName, IsSignal,(1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2] );  
											if(ttbarmass > 0) MyhistoManager.FillHisto2D(Reco_vs_gen_Mttbar_ee_afterbtag1,  "Reco_vs_gen_Mttbar_ee_afterbtag1",  ttbarmass  ,ttbarinvM,  datasetName, IsSignal, weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[2]);
											...
											MyhistoManager.FillHisto(DeltaPhiLLept_ee_afterbtag2, "DeltaPhiLLept_ee_afterbtag2", deltaPhi,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[2] );
											//if ( NBtaggedJets>=2 ){
											MyhistoManager.FillHisto(RecoTopMass_ee_afterbtag2, "RecoTopMass_ee_afterbtag2", topmass,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[2] );
											MyhistoManager.FillHisto(RecoMttbar_ee_afterbtag2, "RecoMttbar_ee_afterbtag2", ttbarmass,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[2] );
											//}
											MyhistoManager.FillHisto(HNjets_ee_aftermetbtag1,   "Njets_ee_aftermetbtag1",    JetMul ,datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
											MyhistoManager.FillHisto(ttbarTransverseMass_ee_afterbtag1, "ttbarTransverseMass_ee_afterbtag1", mttbar_mt, datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
											...
											MyhistoManager.FillHisto(DeltaPhiLLept_emu_afterbtag1, "DeltaPhiLLept_emu_afterbtag1", deltaPhi,  datasetName,  IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]);
                                                                                        MyhistoManager.FillHisto(DeltaPhiLLept_topPtReweighted_emu_afterbtag1,"DeltaPhiLLept_topPtReweighted_emu_afterbtag1",deltaPhi,  datasetName,    IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]*global_weightTauPol*globalTopPtWeight);
                                                                                        MyhistoManager.FillHisto(ptWheightsUsed_emu_after1btag, "ptWheightsUsed_emu_after1btag", globalTopPtWeight, datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]*global_weightTauPol);//addedBy_addHistToFWK.sh_130521_121514
											MyhistoManager.FillHisto(DeltaPhiLLept_emu_afterbtag1_tauPolReweighted, "DeltaPhiLLept_emu_afterbtag1_tauPolReweighted", deltaPhi,  datasetName,  IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]*global_weightTauPol);
											MyhistoManager.FillHisto(tauPolWeights_emu_afterbtag1,"tauPolWeights_emu_afterbtag1",global_weightTauPol,  datasetName, IsSignal, Dweight[ITypeMC]);
											MyhistoManager.FillHisto(cosPhiLLept_emu_afterbtag1,   "cosPhiLLept_emu_afterbtag1",   cosPhi,    datasetName,  IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]);
											MyhistoManager.FillHisto(theO1_emu_afterbtag1,   "theO1_emu_afterbtag1",   theO1,    datasetName,  IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]);
											//if ( NBtaggedJets>=1 ){
											MyhistoManager.FillHisto(RecoTopMass_emu_afterbtag1, "RecoTopMass_emu_afterbtag1", topmass,  datasetName, IsSignal,(1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1] );
											MyhistoManager.FillHisto(RecoMttbar_emu_afterbtag1, "RecoMttbar_emu_afterbtag1", ttbarmass,  datasetName, IsSignal,(1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1] );

											if(ttbarmass > 0) MyhistoManager.FillHisto2D(Reco_vs_gen_Mttbar_emu_afterbtag1,  "Reco_vs_gen_Mttbar_emu_afterbtag1",  ttbarmass  ,ttbarinvM,  datasetName, IsSignal, weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[2]);
											// }
											...
											MyhistoManager.FillHisto(DeltaPhiLLept_emu_afterbtag2, "DeltaPhiLLept_emu_afterbtag2", deltaPhi,  datasetName,    IsSignal, weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[1] );
											//if ( NBtaggedJets>=2 ){
											MyhistoManager.FillHisto(RecoTopMass_emu_afterbtag2, "RecoTopMass_emu_afterbtag2", topmass,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[1] );
											MyhistoManager.FillHisto(RecoMttbar_emu_afterbtag2, "RecoMttbar_emu_afterbtag2", ttbarmass,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[1] );

											//}
											MyhistoManager.FillHisto(HNjets_emu_aftermetbtag1,  "Njets_emu_aftermetbtag1",   JetMul ,datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]);
											MyhistoManager.FillHisto(ttbarTransverseMass_emu_afterbtag1, "ttbarTransverseMass_emu_afterbtag1", mttbar_mt, datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
										}
										else if(CandType=="mumu" && ChannelName=="mumu") {
											...
											MyhistoManager.FillHisto(DeltaPhiLLept_mumu_afterbtag1, "DeltaPhiLLept_mumu_afterbtag1", deltaPhi,  datasetName,    IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0]);
                                                                                        MyhistoManager.FillHisto(DeltaPhiLLept_topPtReweighted_mumu_afterbtag1,"DeltaPhiLLept_topPtReweighted_mumu_afterbtag1",deltaPhi,  datasetName,    IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0]*global_weightTauPol*globalTopPtWeight);
											MyhistoManager.FillHisto(DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted, "DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted", deltaPhi,  datasetName,    IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0]*global_weightTauPol);
											MyhistoManager.FillHisto(tauPolWeights_mumu_afterbtag1,"tauPolWeights_mumu_afterbtag1",global_weightTauPol,  datasetName, IsSignal, Dweight[ITypeMC]);
                                                                                        MyhistoManager.FillHisto(ptWheightsUsed_mumu_after1btag, "ptWheightsUsed_mumu_after1btag", globalTopPtWeight, datasetName, IsSignal,  (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[1]*global_weightTauPol);//addedBy_addHistToFWK.sh_130521_120454

											MyhistoManager.FillHisto(cosPhiLLept_mumu_afterbtag1,   "cosPhiLLept_mumu_afterbtag1",   cosPhi,  datasetName,    IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0]);
											MyhistoManager.FillHisto(theO1_mumu_afterbtag1,   "theO1_mumu_afterbtag1",   theO1,  datasetName,    IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0]);
											//if ( NBtaggedJets>=1 ){
											MyhistoManager.FillHisto(RecoTopMass_mumu_afterbtag1, "RecoTopMass_mumu_afterbtag1", topmass,  datasetName, IsSignal,(1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0] );
											MyhistoManager.FillHisto(RecoMttbar_mumu_afterbtag1, "RecoMttbar_mumu_afterbtag1", ttbarmass,  datasetName, IsSignal,(1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0] );
											if( ttbarmass> 0)  MyhistoManager.FillHisto2D(Reco_vs_gen_Mttbar_mumu_afterbtag1,  "Reco_vs_gen_Mttbar_mumu_afterbtag1",  ttbarmass  ,ttbarinvM,  datasetName, IsSignal, weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[2]);
											...
											MyhistoManager.FillHisto(DeltaPhiLLept_mumu_afterbtag2, "DeltaPhiLLept_mumu_afterbtag2", deltaPhi,  datasetName,
													IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[0] );
											if ( NBtaggedJets>=2 ){
												MyhistoManager.FillHisto(RecoTopMass_mumu_afterbtag2, "RecoTopMass_mumu_afterbtag2", topmass,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[0] );
												MyhistoManager.FillHisto(RecoMttbar_mumu_afterbtag2, "RecoMttbar_mumu_afterbtag2", ttbarmass,  datasetName, IsSignal,weightb[0]*Dweight[ITypeMC]*weight_SFtotal_2btag[0] );

											}
											MyhistoManager.FillHisto(HNjets_mumu_aftermetbtag1, "Njets_mumu_aftermetbtag1",  JetMul ,datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[0]);
											MyhistoManager.FillHisto(ttbarTransverseMass_mumu_afterbtag1, "ttbarTransverseMass_mumu_afterbtag1", mttbar_mt, datasetName, IsSignal, (1-weightb[1])*Dweight[ITypeMC]*weight_SFtotal_1btag[2]);
	...
	/*if(neventsel%100 == 0 && neventsel > 0 ){ cout << "neventsel  " <<  neventsel << endl;
	  cout << "neventsel_recofailure  " << neventsel_recofailure << endl;
	  cout << " neventsel_selfailure " << neventsel_selfailure << endl;}*/
	//if (IReweight ) delete   LumiWeights ;


	//cout<<"nevent done"<<endl;  
	return kTRUE;
}
...
void ProofSelectorMyCutFlow::SlaveTerminate()
{
	...
	if (fFile) {
		...
		TupleTreeUnfolding->Write();
		TupleTreeBeforeSelection->Write();
		...
		MyhistoManager.WriteMyHisto(testRand_ee,   "all");;
		MyhistoManager.WriteMyHisto(testRand_mumu,   "all");;
		MyhistoManager.WriteMyHisto(testRand_emu,   "all");;
		...
		MyhistoManager.WriteMyHisto(HNjets_mumu_aftermetbtag1,   "all");  
		MyhistoManager.WriteMyHisto(HNjets_emu_aftermetbtag1,	 "all");  
		MyhistoManager.WriteMyHisto(HNjets_ee_aftermetbtag1,	"all");     



		MyhistoManager.WriteMyHisto(DeltaPhiLLept_mumu_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_ee_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_emu_aftermetcut,    "all");


		MyhistoManager.WriteMyHisto(DeltaPhiLLept_mumu_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_ee_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_emu_afterbtag1,    "all");

		MyhistoManager.WriteMyHisto(DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_ee_afterbtag1_tauPolReweighted,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_emu_afterbtag1_tauPolReweighted,    "all");
                MyhistoManager.WriteMyHisto(DeltaPhiLLept_topPtReweighted_emu_afterbtag1,"all");
                MyhistoManager.WriteMyHisto(DeltaPhiLLept_topPtReweighted_mumu_afterbtag1,"all");
                MyhistoManager.WriteMyHisto(DeltaPhiLLept_topPtReweighted_ee_afterbtag1,"all");


		MyhistoManager.WriteMyHisto(DeltaPhiLLept_mumu_afterbtag2,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_ee_afterbtag2,    "all");
		MyhistoManager.WriteMyHisto(DeltaPhiLLept_emu_afterbtag2,    "all");

		MyhistoManager.WriteMyHisto(tauPol_pion,"all");//addedBy_addHistToFWK.sh_130222_171848
		MyhistoManager.WriteMyHisto(tauPol_lep,"all");//addedBy_addHistToFWK.sh_130222_171953
		MyhistoManager.WriteMyHisto(tauPolReweighted_lep,"all");//addedBy_addHistToFWK.sh_130228_204836
		MyhistoManager.WriteMyHisto(tauPolWeights_lep,"all");  
		MyhistoManager.WriteMyHisto(tauPolWeights_ee_afterbtag1,"all"); 
		MyhistoManager.WriteMyHisto(tauPolWeights_emu_afterbtag1,"all");
		MyhistoManager.WriteMyHisto(tauPolWeights_mumu_afterbtag1,"all");


		MyhistoManager.WriteMyHisto(cosPhiLLept_mumu_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(cosPhiLLept_ee_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(cosPhiLLept_emu_afterbtag1,    "all");


		MyhistoManager.WriteMyHisto(theO1_mumu_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(theO1_ee_afterbtag1,      "all");
		MyhistoManager.WriteMyHisto(theO1_emu_afterbtag1,     "all");


		MyhistoManager.WriteMyHisto(RecoTopMass_mumu_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(RecoTopMass_ee_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(RecoTopMass_emu_aftermetcut,    "all");


		MyhistoManager.WriteMyHisto(RecoTopMass_mumu_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(RecoTopMass_ee_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(RecoTopMass_emu_afterbtag1,    "all");

		MyhistoManager.WriteMyHisto(RecoTopMass_mumu_afterbtag2,    "all");
		MyhistoManager.WriteMyHisto(RecoTopMass_ee_afterbtag2,    "all");
		MyhistoManager.WriteMyHisto(RecoTopMass_emu_afterbtag2,    "all");




		MyhistoManager.WriteMyHisto(RecoMttbar_mumu_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(RecoMttbar_ee_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(RecoMttbar_emu_aftermetcut,    "all");


		MyhistoManager.WriteMyHisto(RecoMttbar_mumu_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(RecoMttbar_ee_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(RecoMttbar_emu_afterbtag1,    "all");

		MyhistoManager.WriteMyHisto(RecoMttbar_mumu_afterbtag2,    "all");
		MyhistoManager.WriteMyHisto(RecoMttbar_ee_afterbtag2,    "all");
		MyhistoManager.WriteMyHisto(RecoMttbar_emu_afterbtag2,    "all");




		MyhistoManager.WriteMyHisto(CSVDiscri_ee_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(CSVDiscri_emu_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(CSVDiscri_mumu_aftermetcut,    "all");

		MyhistoManager.WriteMyHisto(CSVDiscriBjets_ee_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(CSVDiscriBjets_emu_aftermetcut,    "all");
		MyhistoManager.WriteMyHisto(CSVDiscriBjets_mumu_aftermetcut,    "all");

		MyhistoManager.WriteMyHisto(ttbarTransverseMass_mumu_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(ttbarTransverseMass_ee_afterbtag1,    "all");
		MyhistoManager.WriteMyHisto(ttbarTransverseMass_emu_afterbtag1,    "all");


		MyhistoManager.WriteMyHisto(GEN_ttbarTransverseMass_mumu_afterbtag1, "all"); 
		MyhistoManager.WriteMyHisto(GEN_ttbarTransverseMass_ee_afterbtag1,   "all");  
		MyhistoManager.WriteMyHisto(GEN_ttbarTransverseMass_emu_afterbtag1,  "all");  

		MyhistoManager.WriteMyHisto(GEN_ttbarTransverseMass,    "all");  

		MyhistoManager.WriteMyHisto2D(GEN_ttbarTransverseMass_vs_mttbar,  "all");


		MyhistoManager.WriteMyHisto2D(GENttbarMT_vs_RECOttbarMT_mumu_afterbtag1,  "all");
		MyhistoManager.WriteMyHisto2D(GENttbarMT_vs_RECOttbarMT_ee_afterbtag1,  "all");
		MyhistoManager.WriteMyHisto2D(GENttbarMT_vs_RECOttbarMT_emu_afterbtag1,  "all");



		MyhistoManager.WriteMyHisto2D(Reco_vs_gen_Mttbar_mumu_afterbtag1,  "all");
		MyhistoManager.WriteMyHisto2D(Reco_vs_gen_Mttbar_ee_afterbtag1,  "all");
		MyhistoManager.WriteMyHisto2D(Reco_vs_gen_Mttbar_emu_afterbtag1,  "all");
    /////////////////////////////////////
    // added histos by script: write statement
    /////////////////////////////////////
    MyhistoManager.WriteMyHisto(ptWheightsUsed_mumu_after1btag,"all");//addedBy_addHistToFWK.sh_130521_120454
    MyhistoManager.WriteMyHisto(ptWheightsUsed_emu_after1btag,"all");//addedBy_addHistToFWK.sh_130521_121514
    MyhistoManager.WriteMyHisto(ptWheightsUsed_ee_after1btag,"all");//addedBy_addHistToFWK.sh_130521_121654
    //HereWillScriptAddItWrite
    // end of section with histos added by script
		...
		fHist->Write();
		mcEventWeightHist->Write();
		fFile->Write();
		fOutput->Add(fProofFile);



                DeltaPhiLLept_mumu_afterbtag1.clear();
                DeltaPhiLLept_ee_afterbtag1.clear();
                DeltaPhiLLept_emu_afterbtag1.clear();

                DeltaPhiLLept_mumu_afterbtag1_tauPolReweighted.clear();
                DeltaPhiLLept_ee_afterbtag1_tauPolReweighted.clear();
                DeltaPhiLLept_emu_afterbtag1_tauPolReweighted.clear();
                DeltaPhiLLept_topPtReweighted_emu_afterbtag1.clear();
                DeltaPhiLLept_topPtReweighted_mumu_afterbtag1.clear();
                DeltaPhiLLept_topPtReweighted_ee_afterbtag1.clear();
		...
		DYMassCR_ee.clear();
		DYMassSR_ee.clear();
		tauPol_pion.clear();//addedBy_addHistToFWK.sh_130222_171848
		tauPol_lep.clear();//addedBy_addHistToFWK.sh_130222_171953
		tauPolReweighted_lep.clear();//addedBy_addHistToFWK.sh_130228_204836   
		tauPolWeights_lep.clear(); 
		tauPolWeights_ee_afterbtag1.clear();
		tauPolWeights_emu_afterbtag1.clear();
		tauPolWeights_mumu_afterbtag1.clear();
		pileup.clear();
		datasets.clear();
    ////////////////////////////////////////
    // added histos by script : delete/clear statement
    //////////////////////////////////////////
    ptWheightsUsed_mumu_after1btag.clear();//addedBy_addHistToFWK.sh_130521_120454
    ptWheightsUsed_emu_after1btag.clear();//addedBy_addHistToFWK.sh_130521_121514
    ptWheightsUsed_ee_after1btag.clear();//addedBy_addHistToFWK.sh_130521_121654
    //HereWillScriptAddClearing
    // end of histos added by script
	...
	TString ProofSelectorMyCutFlow::getIndexPDF(int i){
	...
	void ProofSelectorMyCutFlow::fillPDFHisto(double deltaPhi, string datasetname, bool IsSignal, double weight, std::vector < float > pdfWeights 
			, TString plotname, std::vector<TH1F>  * vectorHisto){

		if(IsSignal){
			//cout << "fill pdf histo" << endl;
			for (int i = 0;i<41;++i) {
				TString index = getIndexPDF(i);     

				if(plotname == "DeltaPhiLLept_mumu_aftermetcut" ) (DeltaPhiLLept_mumu_aftermetcut_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
				if(plotname == "DeltaPhiLLept_ee_aftermetcut"   ) (DeltaPhiLLept_ee_aftermetcut_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
				if(plotname == "DeltaPhiLLept_emu_aftermetcut"  ) (DeltaPhiLLept_emu_aftermetcut_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);


				if(plotname == "DeltaPhiLLept_mumu_afterbtag1" ) (DeltaPhiLLept_mumu_afterbtag1_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
				if(plotname == "DeltaPhiLLept_ee_afterbtag1"   ) (DeltaPhiLLept_ee_afterbtag1_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
				if(plotname == "DeltaPhiLLept_emu_afterbtag1"  ) (DeltaPhiLLept_emu_afterbtag1_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);

				if(plotname == "DeltaPhiLLept_mumu_afterbtag2" ) (DeltaPhiLLept_mumu_afterbtag2_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
				if(plotname == "DeltaPhiLLept_ee_afterbtag2"   ) (DeltaPhiLLept_ee_afterbtag2_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
				if(plotname == "DeltaPhiLLept_emu_afterbtag2"  ) (DeltaPhiLLept_emu_afterbtag2_pdf)[i].Fill(deltaPhi, weight*pdfWeights[i]);
			}
	...
	std::vector < float > ProofSelectorMyCutFlow::getPDFweights(double x1, double x2, double Q_scale, int partonFlavor1, int partonFlavor2){

		//cout << "caclul pdf weight" << endl;
		unsigned int nweights = LHAPDF::numberPDF();
		//unsigned int nweights = 0;
		double pdf1, pdf2;
		std::vector < float >  weights;
		for (unsigned int i=0; i<=nweights; ++i) {
			LHAPDF::usePDFMember(i);
			double newpdf1 = LHAPDF::xfx(x1, Q_scale, partonFlavor1)/x1;
			double newpdf2 = LHAPDF::xfx(x2, Q_scale, partonFlavor2)/x2;
			if (i==0) {
				pdf1 = newpdf1; pdf2 = newpdf2;
			}
			weights.push_back(newpdf1/pdf1*newpdf2/pdf2);
			//cout << "    the weight is  " << newpdf1/pdf1*newpdf2/pdf2 << endl;
		}
