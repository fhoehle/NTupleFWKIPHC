...
void Plot2_main(string observable, string outname){
  ...
  */
  ...
  for (int ilum=0; ilum<lumiband->GetNbinsX(); ilum++) {    
	  double   error_all = 
	      pow(
		  (histo1D_ttSig->GetBinContent(ilum+1)+histo1D_VV->GetBinContent(ilum+1)+histo1D_tW->GetBinContent(ilum+1))
		  *lumi_error, 
		  2)+
		  ...
		  *SF_MET_error
		  , 2);
	      pow(
		  (histo1D_VV->GetBinContent(ilum+1)+histo1D_tW->GetBinContent(ilum+1))
		  *0.30
		  , 2);
  ...
  */
  ...
  qw->AddEntry(histo1D_ttBkg,     "fake lepton  "     ,"f");
