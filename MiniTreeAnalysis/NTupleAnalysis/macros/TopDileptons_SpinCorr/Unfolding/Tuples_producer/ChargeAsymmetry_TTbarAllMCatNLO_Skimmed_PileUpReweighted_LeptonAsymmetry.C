...
// fixme: Depracated
  ...
  TFile * TupleFile = new TFile("TupleFileSkimmed_TTbarAllMCatNLO_PileUpReweighted_LeptonAsymmetry.root","RECREATE");
  ...
  TFile* file1  = new TFile("../data/dataPUhisto_2011AB_73.5mb_pixelLumi.root","READ"); 
  TH1D *  hPUData = 0;
...
*/
      ...
      if(thePUReweighter.getUseOutOfTimePU()){
         weight *= thePUReweighter.weight(event->num_pileup_bc0, event->runNb);
      }else{
         weight *= thePUReweighter.weight(event->num_pileup_bc0);
      }
	...
	false, false, -1., -1., applyJES,  JESParam,
	 ...
	 applyJER,  JERFactor,  applyMETS,  METScale);
        ...
        lvTop1_gen = (event->wAndDecays)[0].p4_Lep_gen;
        lvTop2_gen = (event->wAndDecays)[1].p4_Lep_gen;
...
//        TopMassVariables * tmv = amwt.findMass(sel, lvTop1_reco, lvTop2_reco);
        ...
        for(unsigned int i=0; i<candMuon.size(); i++){
          if(candMuon[i].Charge == +1) lvTop1_reco = candMuon[i].p4;
          if(candMuon[i].Charge == -1) lvTop2_reco = candMuon[i].p4;
        }
        
        for(unsigned int j=0; j<candElec.size(); j++){
          if(candElec[j].Charge == +1) lvTop1_reco = candElec[j].p4;
          if(candElec[j].Charge == -1) lvTop2_reco = candElec[j].p4;
        }
...
//  }
    ...
    TFile * fileCA = new TFile("ChargeAsymmetryResults_Skimmed_TTbarAllMCatNLO_PileUpReweighted_LeptonAsymmetry.root","RECREATE");
    //  fileCA->cd();
