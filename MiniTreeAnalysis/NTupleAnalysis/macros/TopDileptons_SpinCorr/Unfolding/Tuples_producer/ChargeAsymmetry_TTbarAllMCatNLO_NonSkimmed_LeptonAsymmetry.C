...
// fixme: Depracated
  ...
  TFile * TupleFile = new TFile("TupleFileNonSkimmed_TTbarAllMCatNLO_LeptonAsymmetry.root","RECREATE");
...
*/
	...
	false, false, -1., -1., applyJES,  JESParam,
	 ...
	 applyJER,  JERFactor,  applyMETS,  METScale);
        ...
        cout << "(event->wAndDecays)[0].p4_Lep_gen.Perp(): " << (event->wAndDecays)[0].p4_Lep_gen.Perp() << endl; 
        cout << "(event->wAndDecays)[7].p4_Lep_gen.Perp(): " << (event->wAndDecays)[7].p4_Lep_gen.Perp() << endl; 
...
//  }
    ...
    TFile * fileCA = new TFile("ChargeAsymmetryResults_NonSkimmed_TTbarAllMCatNLO_LeptonAsymmetry.root","RECREATE");
    //  fileCA->cd();
