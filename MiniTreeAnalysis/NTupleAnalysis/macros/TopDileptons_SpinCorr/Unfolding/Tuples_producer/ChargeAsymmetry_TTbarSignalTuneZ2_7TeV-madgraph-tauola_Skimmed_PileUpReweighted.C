...
// fixme: Depracated
  ...
  TFile * TupleFile = new TFile("TupleFileSkimmed_TTbarSignalTuneZ2_7TeV-madgraph-tauola_PileUpReweighted.root","RECREATE");
...
//           for (unsigned int ievt = 0; ievt < 10; ievt++) {
      ...
      if(!(event->TMEME == 2  || event->TMEME == 10101 || event->TMEME == 20200 || event->TMEME == 20 || event->TMEME == 11010 || event->TMEME == 22000|| event->TMEME == 11 || event->TMEME == 11001 || event->TMEME == 10110 || event->TMEME == 21100 )) continue;
...
//  }
    ...
    TFile * fileCA = new TFile("ChargeAsymmetryResults_Skimmed_TTbarSignalTuneZ2_7TeV-madgraph-tauola_PileUpReweighted.root","RECREATE");
    //  fileCA->cd();
