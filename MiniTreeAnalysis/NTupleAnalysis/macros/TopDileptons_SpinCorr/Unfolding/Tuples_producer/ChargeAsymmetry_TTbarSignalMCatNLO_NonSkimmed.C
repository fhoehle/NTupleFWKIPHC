...
// fixme: Depracated
  ...
  TFile * TupleFile = new TFile("TupleFileNonSkimmed_TTbarSignalMCatNLO.root","RECREATE");
...
//           for (unsigned int ievt = 0; ievt < 10; ievt++) {
	...
	false, false, -1., -1., applyJES,  JESParam,
	 ...
	 applyJER,  JERFactor,  applyMETS,  METScale);
        ...
        lvTop1_gen = (event->topAndDecays)[6].p4_t_gen;
        lvTop2_gen = (event->topAndDecays)[7].p4_t_gen;
...
//  }
    ...
    TFile * fileCA = new TFile("ChargeAsymmetryResults_NonSkimmed_TTbarSignalMCatNLO.root","RECREATE");
    //  fileCA->cd();
