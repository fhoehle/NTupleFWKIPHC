...
void ZPeakNorm(bool useMuMu){
  ...
  TFile *f_data  = new TFile("../backup_outputProof20-03-12_19-24-49/proof.root");
  f_data->cd();
  ...
  AllZjets_ee  ->Scale(1./0.962);
  AllZjets_mumu->Scale(1./0.977);
  InZpeak_ee   ->Scale(1./0.962);
  InZpeak_mumu ->Scale(1./0.977);
  ...
  if(useMuMu){
   ...
   //*********************************
    ...
    cout << "N_predicted from mumu " << N_predicted << endl;
    cout << "N_Data      " << N_Data      << endl; 
    cout << "lumi scale  " << N_Data/N_predicted  << " +/- "<<  error<< endl;
    cout << "new nlumi   " << (N_Data/N_predicted)*2.268  << " +/- "<<  (N_Data/N_predicted)*error<< endl;
  ...
  if(!useMuMu){
    double entrie_ee = InZpeak_ee->Integral()/AllZjets_ee->Integral();
    double Br = 1./3.;
   
    double norm_cross_section =1./(3048.*entrie_ee*Br);
    double ewk_crosssection = 992.0;
    cout << "norm_cross_section "  <<  norm_cross_section << endl;
    
    //cout << "  " << InZpeak_ee->Integral()*norm_cross_section*ewk_crosssection << endl;
    double N_predicted = InZpeak_ee->Integral()*norm_cross_section*ewk_crosssection;
   
   
   
    //double N_Data =  InZpeak_ee_data->Integral(1, InZpeak_ee_data->GetNbinsX());
    double N_Data =  InZpeak_ee_data->Integral();
    
   //*********************************
   // calculate error
   //*********************************
    double errorXS = InZpeak_ee->Integral()*norm_cross_section*48.0;
    double error = pow( pow((1/N_predicted)*pow(N_Data,0.5),2) +  pow((N_Data/(N_predicted*N_predicted))*errorXS,2), 0.5);
    
    
     
    cout << "N_predicted from ee " << N_predicted << endl;
    cout << "N_Data      " << N_Data      << endl; 
    cout << "lumi scale  " << N_Data/N_predicted  << " +/- "<<  error<< endl;
    cout << "new nlumi   " << (N_Data/N_predicted)*2.253  << " +/- "<<  (N_Data/N_predicted)*error<< endl;
