...
int main(int argc, char* argv[]){
 ...
 string xmlFileName = "";
   if (configFile != NULL){
        cout<<"using non default configFile: "<<configFile<<std::endl;
 ...
 //string xmlFileName = string("/opt/sbg/data/data1/cms/jandrea/TopIPHC_2012_02_03/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/config/MyCutFlow.xml");  
//  string xmlFileName =
// 
//string("/opt/sbg/data/safe1/cms/jandrea/TTbarSpinCorrelation_7TeV/CMSSW_4_2_8_patch7/src/MiniTreeAnalysis/NTupleAnalysis/config/MyCutFlow_2011AB.xml");  
string outputFileName = "proof.root";
  ...
  TProof *proof = TProof::Open(""); //jandrea@sbgui5.in2p3.fr:21001");
  ...
  time_t rawtime; struct tm *timeinfo; char timeStamp [80];
  time(&rawtime); timeinfo = localtime(&rawtime);
  strftime(timeStamp,80,"%y-%m-%d_%H-%M-%S",timeinfo);
  std::string directory  = std::string("backup_outputProof_")+std::string(timeStamp);
  system((std::string("mkdir ") + directory +std::string("; mv proof*.root ")+directory+std::string("/")).c_str());
