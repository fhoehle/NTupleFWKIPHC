...
void PlotStack(TString plotname, TString namechan, TString selection, bool setlogy, bool ratio){
  ...
  TString filename;
  filename="backup_outputProof20-02-13_10-03-01//proof.root";
  ...
  */
  ...
  if(readStop){
    TString histo_STOP_name = plotname+channel+selection+"_STOP";
    TH1F * histo_STOP                  = (TH1F*)filechannel->Get(histo_STOP_name);
    if (namechan=="all") {
      TString h_name2_STOP=plotname+"mumu"+selection+"_STOP";
      TH1F * h_2_STOP = (TH1F*)filechannel->Get(h_name2_STOP);
      TString h_name3_STOP=plotname+"emu"+selection+"_STOP";
      TH1F * h_3_STOP = (TH1F*)filechannel->Get(h_name3_STOP);
      histo_STOP->Add(histo_STOP, h_2_STOP     , 1, 1);
      histo_STOP->Add(histo_STOP, h_3_STOP     , 1, 1);
    }
  }
  ...
  for (int ilum=0; ilum<lumiband->GetNbinsX(); ilum++) {
    ...
    double error_all = 
       pow(
         (histo_TTbarSig->GetBinContent(ilum+1)+histo_VV->GetBinContent(ilum+1)+histo_SingleToptW->GetBinContent(ilum+1))
         *lumi_error, 
         2)+
       //*************************
  ...
  if (plotname=="NVxP_" && namechan=="all" && selection=="_aftertrigger") {
    ...
    else if(plotname == "ttbarTransverseMass_")   theXtitle ="m_{T} [GeV]";
  ...
  if(plotname == "NBjets_")hs->SetMaximum(2000);
  if(plotname == "NBjets_" && namechan=="emu")hs->SetMaximum(6000);
  if(plotname == "NBjets_" && namechan=="all")hs->SetMaximum(12000);
  ...
  //latex2->DrawLatex(0.87, 0.95, "CMS 5.0 fb^{-1} at #sqrt{s} = 7 TeV");
  latex2->DrawLatex(0.82, 0.95, "CMS Preliminary 5.0 fb^{-1} at #sqrt{s} = 7 TeV");
  ...
  if(plotname == "NBjets_"  || plotname == "Njets_") qw = new TLegend(.20,.60,.45,.85);
  else{ 
    if(plotname == "Met_")    qw = new TLegend(.65,.55,.95,.90);
    else  qw = new TLegend(.60,.55,.95,.90);
  }
  if(plotname ==  "DeltaPhiLLept_")  qw = new TLegend(.20,.60,.45,.85);
