...
//  string ChannelName  = "ee";  // "mumu", "ee", "emu"
  ...
  bool IReweight        = true;
  bool IReweight_Spring11 = true;
  bool IDYestimateWithMetCut = true;
// Here define Scale Factors
  //SF_trigger applied for mumu
//  double SF_trig_mu=0.977;  // for 2010 Run
  double SF_trig_mu=1.0;  // for 2011 Run
...
//  double SF_e=0.965; // for 2010 Run
  ...
  std::vector<TH1F> HDeltaPhi_emu_pair_aftermetcut;
  std::vector<TH1F> HDeltaR_emu_pair_aftermetcut;
  std::vector<TH1F> HDeltaPhi_leptMET_emu_pair_aftermetcut;
  ...
  std::vector<TH1F> HDeltaPhi_mumu_pair_aftermetcut;
  std::vector<TH1F> HDeltaR_mumu_pair_aftermetcut;
  std::vector<TH1F> HDeltaPhi_leptMET_mumu_pair_aftermetcut;
  ...
  std::vector<TH1F> HDeltaPhi_ee_pair_aftermetcut;
  std::vector<TH1F> HDeltaR_ee_pair_aftermetcut;
  std::vector<TH1F> HDeltaPhi_leptMET_ee_pair_aftermetcut;
  ...
  /*if (IReweightWithVxP ) {
     TFile* fin1  = new TFile("VxP_Weight_MuEG.root","READ");
     ...
     fin1->Close();
     TFile* fin2  = new TFile("VxP_Weight_Mu.root","READ");
     ...
     fin2->Close();
     TFile* fin3  = new TFile("VxP_Weight_EG.root","READ");
...
// Creation of histogram list
  ...
  for (unsigned int d = 0; d < datasets.size (); d++) {
    ...
    MyhistoManager.CreateHisto(HDeltaPhi_emu_pair_aftermetcut,       "HDeltaPhi_emu_pair_aftermetcut",      
    datasets[d].Name(),"delta phi","Entries",100,0.,5);;
    MyhistoManager.CreateHisto(HDeltaR_emu_pair_aftermetcut,         "HDeltaR_emu_pair_aftermetcut",        
    datasets[d].Name(),"delta R","Entries",100,0.,5);
    MyhistoManager.CreateHisto(HDeltaPhi_leptMET_emu_pair_aftermetcut, "HDeltaPhi_leptMET_emu_pair_aftermetcut",
    datasets[d].Name(),"delta R","Entries",100,0.,5);
    ...
    MyhistoManager.CreateHisto(HDeltaPhi_mumu_pair_aftermetcut,       "HDeltaPhi_mumu_pair_aftermetcut",      
    datasets[d].Name(),"delta phi","Entries",100,0.,5);;
    MyhistoManager.CreateHisto(HDeltaR_mumu_pair_aftermetcut,         "HDeltaR_mumu_pair_aftermetcut",        
    datasets[d].Name(),"delta R","Entries",100,0.,5);
    MyhistoManager.CreateHisto(HDeltaPhi_leptMET_mumu_pair_aftermetcut, "HDeltaPhi_leptMET_mumu_pair_aftermetcut",
    datasets[d].Name(),"delta R","Entries",100,0.,5);
    ...
    MyhistoManager.CreateHisto(HDeltaPhi_ee_pair_aftermetcut,       "HDeltaPhi_ee_pair_aftermetcut",      
    datasets[d].Name(),"delta phi","Entries",10,0.,5);
    MyhistoManager.CreateHisto(HDeltaR_ee_pair_aftermetcut,         "HDeltaR_ee_pair_aftermetcut",        
    datasets[d].Name(),"delta R","Entries",100,0.,5);
    MyhistoManager.CreateHisto(HDeltaPhi_leptMET_ee_pair_aftermetcut, "HDeltaPhi_leptMET_ee_pair_aftermetcut",
    datasets[d].Name(),"delta R","Entries",100,0.,5);
    ...
    MyhistoManager.CreateHisto(pileup,                "pileup"            ,datasets[d].Name(),"pileup","Entries",31,-0.5,30.5);
...
//      for (int IChannel=1; IChannel<2; IChannel++) {
	    ...
	    if(IReweight ){
	        if(thePUReweighter.getUseOutOfTimePU()) weightITypeMC = weightITypeMC_save*thePUReweighter.weight(event->num_pileup_bc0, event->runNb);
	        else{
		  if( IReweight_Spring11) weightITypeMC = weightITypeMC_save*thePUReweighter.weight_Spring11(event->num_pileup_bc0);
		  else weightITypeMC = weightITypeMC_save*thePUReweighter.weight(event->num_pileup_bc0);
		}
	    }
...
//     Passage trigger
      // Voir DileptonSelection.cc pour conditions de trigger
	   ...
	   TabFlow2[IChannel][ITypeMC][10]+=Dweight[ITypeMC]*Dweight[ITypeMC];
           if (ChannelName=="mumu" &&  !IsData ) Dweight[ITypeMC]*=SF_trig_mu;
           if (ChannelName=="emu"  &&  !IsData ) Dweight[ITypeMC]*=SF_trig_emu;
	   ...
	   sel.GetLeptonPairLikeSign(candMuon, candElec, CandType ); 
...
//              if ( datasets[d].Name()=="WjetsE"  || datasets[d].Name()=="WjetsTau" ) --> hyp : real mu, fake e
              if ( datasets[d].Name()=="SingleToptW" ) Dweight[ITypeMC] = Dweight[ITypeMC]*SF_e;
          }
...
// Passage met
		...
		if(checkDuplicate[event->eventNb] == event->runNb)
		    cout << "duplicate evt" << event->eventNb << "  run " << event->runNb << endl;
		else checkDuplicate[event->eventNb] == event->runNb;
		...
		if(CandType=="mumu" && ChannelName=="mumu"){ 
	         ...
	         double deltaPhill = candMuon[0].p4.DeltaPhi(candMuon[1].p4);
	         double deltaRll = candMuon[0].p4.DeltaR(candMuon[1].p4);
	         double deltaPhi1lmet = candMuon[0].p4.DeltaR(met.p4);
	         double deltaPhi2lmet = candMuon[1].p4.DeltaR(met.p4);
		 MyhistoManager.FillHisto(HDeltaPhi_mumu_pair_aftermetcut,       "HDeltaPhi_mumu_pair_aftermetcut" , deltaPhill ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 MyhistoManager.FillHisto(HDeltaR_mumu_pair_aftermetcut,         "HDeltaR_mumu_pair_aftermetcut" , deltaRll ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 MyhistoManager.FillHisto(HDeltaPhi_leptMET_mumu_pair_aftermetcut, "HDeltaPhi_leptMET_mumu_pair_aftermetcut" , deltaPhi1lmet ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
	         MyhistoManager.FillHisto(HDeltaPhi_leptMET_mumu_pair_aftermetcut, "HDeltaPhi_leptMET_mumu_pair_aftermetcut" , deltaPhi2lmet ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 ...
		 MyhistoManager.FillHisto(HInvM_mumu_pair_aftermetcut, "InvmumuMassPair_aftermetcut" ,InvDilMass,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
	        ...
	        if(CandType=="ee" && ChannelName=="ee"){
	         double deltaPhill = candElec[0].p4.DeltaPhi(candElec[1].p4);
	         double deltaRll = candElec[0].p4.DeltaR(candElec[1].p4);
	         double deltaPhi1lmet = candElec[0].p4.DeltaR(met.p4);
	         double deltaPhi2lmet = candElec[1].p4.DeltaR(met.p4);
		 MyhistoManager.FillHisto(HDeltaPhi_ee_pair_aftermetcut,       "HDeltaPhi_ee_pair_aftermetcut" , deltaPhill ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 MyhistoManager.FillHisto(HDeltaR_ee_pair_aftermetcut,         "HDeltaR_ee_pair_aftermetcut" , deltaRll ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 MyhistoManager.FillHisto(HDeltaPhi_leptMET_ee_pair_aftermetcut, "HDeltaPhi_leptMET_ee_pair_aftermetcut" , deltaPhi1lmet ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
	         MyhistoManager.FillHisto(HDeltaPhi_leptMET_ee_pair_aftermetcut, "HDeltaPhi_leptMET_ee_pair_aftermetcut" , deltaPhi2lmet ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 ...
		 double deltaPhill = candElec[0].p4.DeltaPhi(candMuon[0].p4);
	         double deltaRll = candElec[0].p4.DeltaR(candElec[0].p4);
	         double deltaPhi1lmet = candElec[0].p4.DeltaR(met.p4);
	         double deltaPhi2lmet = candMuon[0].p4.DeltaR(met.p4);
		 MyhistoManager.FillHisto(HDeltaPhi_emu_pair_aftermetcut,       "HDeltaPhi_emu_pair_aftermetcut" , deltaPhill ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 MyhistoManager.FillHisto(HDeltaR_emu_pair_aftermetcut,         "HDeltaR_emu_pair_aftermetcut" , deltaRll ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
		 MyhistoManager.FillHisto(HDeltaPhi_leptMET_emu_pair_aftermetcut, "HDeltaPhi_leptMET_emu_pair_aftermetcut" , deltaPhi1lmet ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
	         MyhistoManager.FillHisto(HDeltaPhi_leptMET_emu_pair_aftermetcut, "HDeltaPhi_leptMET_emy_pair_aftermetcut" , deltaPhi2lmet ,datasets[d].Name()   , IsSignal, Dweight[ITypeMC]);
...
// "MC" or "DATA" origin for the scale factor (NofEventsintheMZpieak(inSR)/NofEventsintheMZpieak(inCR))
  ...
  std::cout<<"systematical error (counting) =  "<<DYest_mumu.GetNDYEstimatedSysErrorDATA() <<std::endl;
  float ert0 = sqrt(sterc*sterc+DYest_mumu.GetNDYEstimatedSysErrorDATA()*DYest_mumu.GetNDYEstimatedSysErrorDATA());
...
// "MC" or "DATA" origin for the scale factor (NofEventsintheMZpieak(inSR)/NofEventsintheMZpieak(inCR))
  ...
  std::cout<<"total error  (fitting)      =  "<< ert1 <<std::endl;
  std::cout<<"statistical error (counting)  =  "<<sterc <<std::endl;
  std::cout<<"systematical error (counting) =  "<<DYest_ee.GetNDYEstimatedSysErrorDATA() <<std::endl;
  ert0 = sqrt(sterc*sterc+DYest_ee.GetNDYEstimatedSysErrorDATA()*DYest_ee.GetNDYEstimatedSysErrorDATA());
  std::cout<<"total error  (counting)       =  "<< ert0 <<std::endl;
  std::cout<<"======== "<<std::endl; 
  std::cout<<""<<std::endl; 
...
// ajout Caro
  ...
  ofile << "$SF_{eID}$  & " << TabFlow1[IChannel][100][11] << " & " <<
...
// ajout Caro
  ...
  ofile << "$SF_{eID}$  & " << TabFlow1[IChannel][1][11] <<  " & "
...
//  }
  ...
  MyhistoManager.WriteMyHisto(HDeltaPhi_ee_pair_aftermetcut,"all");
  MyhistoManager.WriteMyHisto(HDeltaR_ee_pair_aftermetcut,"all");
  MyhistoManager.WriteMyHisto(HDeltaPhi_leptMET_ee_pair_aftermetcut,"all");
  
  MyhistoManager.WriteMyHisto(HDeltaPhi_mumu_pair_aftermetcut,"all");
  MyhistoManager.WriteMyHisto(HDeltaR_mumu_pair_aftermetcut,"all");
  MyhistoManager.WriteMyHisto(HDeltaPhi_leptMET_mumu_pair_aftermetcut,"all");
  
  MyhistoManager.WriteMyHisto(HDeltaPhi_emu_pair_aftermetcut,"all");
  MyhistoManager.WriteMyHisto(HDeltaR_emu_pair_aftermetcut,"all");
  MyhistoManager.WriteMyHisto(HDeltaPhi_leptMET_emu_pair_aftermetcut,"all");
