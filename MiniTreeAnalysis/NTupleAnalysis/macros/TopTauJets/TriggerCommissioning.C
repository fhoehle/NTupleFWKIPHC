 ...
 //////////////////////
  ...
  for (unsigned int d = 0; d < datasets.size (); d++) {
    ...
    for (unsigned int ievt = 0; ievt < nEvents; ievt++) {
     ...
     // if ((event->runNb>=165970 && event->runNb<=166782) || event->runNb>=171050 ) continue;
     ...
     vector<NTTau>  selTaus          = sel.GetSelectedTaus(selMuons,selElectrons,false,1,0,true);
     ...
     trigComm.FillHistos(selJets_noClean, selTaus, PassTriggerQuadJet45IsoPFTau,filterStatus,filterStatus2,tauObjTrig,jetObjTrig);
