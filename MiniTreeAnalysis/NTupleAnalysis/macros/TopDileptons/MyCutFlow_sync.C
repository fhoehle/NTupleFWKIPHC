...
void EventDump(NTEvent* event, string candType, vector<NTElectron> candElectron, vector<NTMuon> candMuon, vector<NTJet> jets, NTMET met, float mass){
	float v1,v2,v3,v4=0;
  	float v1a, v1b, v1c, v1d, v1e, v1f, v1g, v1h = 0;
  	float v2a, v2b, v2c, v2d, v2e, v2f, v2g, v2h = 0;
	if(candType == "mumu"){
		v1 =candMuon[0].p4.Pt()*candMuon[0].Charge;
		v2 =candMuon[1].p4.Pt()*candMuon[1].Charge;
		v3 =candMuon[0].RelIso03PF();
		v4 =candMuon[1].RelIso03PF();
	}
	if(candType == "ee"){
		v1 =candElectron[0].p4.Pt()*candElectron[0].Charge;
		v2 =candElectron[1].p4.Pt()*candElectron[1].Charge;
		v3 =candElectron[0].RelIso03PF();
		v4 =candElectron[1].RelIso03PF();
		v1a=candElectron[0].nlost;
		v2a=candElectron[1].nlost;
		v1b=fabs(candElectron[0].deltaCotTheta);
		v2b=fabs(candElectron[1].deltaCotTheta);
		v1c=fabs(candElectron[0].deltaDistance);
		v2c=fabs(candElectron[1].deltaDistance);
 		v1d = candElectron[0].elecIdWP90_c;
 		v2d = candElectron[1].elecIdWP90_c;
 		v1e = candElectron[0].ET_SC;
 		v2e = candElectron[1].ET_SC;
 		v1f = candElectron[0].isGsfElectron;
 		v2f = candElectron[1].isGsfElectron;
 		v1g = candElectron[0].isEcalDriven;
 		v2g = candElectron[1].isEcalDriven;
 		v1h = candElectron[0].D0;
 		v2h = candElectron[1].D0;
	}
	if(candType == "emu"){
		v1 =candElectron[0].p4.Pt()*candElectron[0].Charge;
		v2 =candMuon[0].p4.Pt()*candMuon[0].Charge;
		v3 =candElectron[0].RelIso03PF();
		v4 =candMuon[0].RelIso03PF();
	}
 	//float v5 =sel.DiLeptonMass(candMuon,candElectron);
 	float v5 = mass;
        float v6 = 0.;
        float v7 = 0.;
        float v8 = 0.;
	...
	v8 = met.p4.Et();
	//if(candType=="ee") cout<<candType<<"|"<<event->runNb<<"|"<<event->eventNb<<"|"<<v1<<","<<v2<<"|"<<v3<<","<<v4<<"|"<<v1a<<","<<v2a<<"|"<<v1b<<","<<v2b<<"|"<<v1c<<","<<v2c<<"|"<<v5<<"|"<<v6<<","<<v7<<"|"<<v8<<"|"<<v9<<","<<v10<<std::endl;
	if(candType=="ee"){ 
		cout<<candType<<"|"<<event->runNb<<"|"<<event->eventNb<<"|"<<v1<<","<<v2<<"|"<<v3<<","<<v4<<"|"<<v1a<<","<<v2a<<"|"<<v1b<<","<<v2b<<"|"<<v1c<<","<<v2c;
		cout<<"|"<<v1d<<","<<v2d<<"|"<<v1e<<","<<v2e<<"|"<<v1f<<","<<v2f<<"|"<<v1g<<","<<v2g<<"|"<<v1h<<","<<v2h;
		cout<<"|"<<v5<<"|"<<v6<<","<<v7<<"|"<<v8<<"|"<<v9<<","<<v10<<std::endl;
	}
	else cout<<candType<<"|"<<event->runNb<<"|"<<event->eventNb<<"|"<<v1<<","<<v2<<"|"<<v3<<","<<v4<<"|"<<v5<<"|"<<v6<<","<<v7<<"|"<<v8<<"|"<<v9<<","<<v10<<std::endl;
}
...
//  string ChannelName  = "ee";  // "mumu", "ee", "emu"
  ...
  bool IReweightWithVxP = true;//true
...
// Creation of histogram list
  ...
  for (unsigned int d = 0; d < datasets.size (); d++) {
    ...
    for (unsigned int ievt = 0; ievt < nEvents; ievt++) {
     ...
     /*
     for(unsigned int id=0;id<event->triggers.size();id++){
	cout<<event->triggers[id].first<<" "; 
     }
     cout<<endl;
     return 0;
     */
	...
	//For debugging
 /*
      if(event->eventNb == 1749851){
	cout<<"event "<<event->eventNb<<" TMEME "<<event->TMEME<<endl;
        cout<<"trigger mumu "<<sel.passTriggerSelection ( &datasets[d], string("mumu"))<<endl;
        cout<<"trigger emu "<<sel.passTriggerSelection ( &datasets[d], string("emu"))<<endl;
        cout<<"trigger ee "<<sel.passTriggerSelection ( &datasets[d], string("ee"))<<endl;
	EventDump(event, string("blabla"), selElectrons, selMuons, selJets, met, sel.DiLeptonMass(selMuons,selElectrons));
      }
      else continue;
  */
        /////////////////////////////////
...
//	  if (( InvDilMass<sel.GetZmassWindowCut().first || InvDilMass>sel.GetZmassWindowCut().second ) && (InvDilMass> sel.GetMinValueMassCut() ))  {
	  ...
	  */
	  if ( ( (ChannelName=="mumu" || ChannelName=="ee") && 
	        selVertices.size()>0 && InvDilMass> sel.GetMinValueMassCut() && sel.DiLeptonMassCut(sel.GetMinValueMassCut(),sel.GetZmassWindowCut(),candMuon,candElec,ChannelName) ) || 
		(ChannelName=="emu" && selVertices.size()>0 && InvDilMass> sel.GetMinValueMassCut() && InvDilMass> sel.GetMinValueMassCut())
	       )  {
...
// Passage met
		...
		//For Sync'
		EventDump(event, CandType, candElec, candMuon, theselJets, met, sel.DiLeptonMass(candMuon,candElec));
	        ////////////////////////
