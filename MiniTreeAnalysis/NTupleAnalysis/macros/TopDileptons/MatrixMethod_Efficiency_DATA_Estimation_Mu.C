...
int main ()
{
  ...
  string xmlFileName = string ("../../config/MatrixMethod_DATA_Mu_forEfficiency.xml");
    ...
    for (unsigned int ievt = 0; ievt < nEvents; ievt++){
	 ...
	 jets_size[k]=0;
	  ...
	  if(jets[j].p4.Perp() > 10*k) jets_size[k]++;
       ...
       if(datasets[d].Name() == "Jet"){
        for (unsigned int j = 0; j < muons.size(); j++) {
           Iso_DATA_Jet->Fill(muons[j].RelIso03PF());
        }
        for (unsigned int j = 0; j < jets.size(); j++) {
       ...
       if(muons.size() == 1){
          float phi_l = muons[0].p4.Phi();
          float pT_l = muons[0].p4.Perp();
	  ...
	  //          if(mT < 80 && mT > 65) ainvmasscutW = false;
       ...
       for (unsigned int i = 0; i < 10; i++) {
        if(muons.size() == 1 && met.p4.Perp() < 20 && jets_size[i] >= 1 && ainvmasscutW && datasets[d].Name() == "Jet"){   	    
             if (muons[0].RelIso03PF() < 0.8){ 
                  IsoLooseForFakeRateNJets[i]->Fill(jets_Loose.size());
                  IsoLooseForFakeRateEta[i]->Fill(muons[0].p4.Eta());
                  IsoLooseForFakeRatePT[i]->Fill(muons[0].p4.Perp());
                  IsoLooseForFakeRateNVTX[i]->Fill(selVertices.size());
             }
   	     if (muons[0].RelIso03PF() < 0.2){ 
                  IsoTightForFakeRateNJets[i]->Fill(jets_Tight.size());
                  IsoTightForFakeRateEta[i]->Fill(muons[0].p4.Eta());
                  IsoTightForFakeRatePT[i]->Fill(muons[0].p4.Perp());
                  IsoTightForFakeRateNVTX[i]->Fill(selVertices.size());
             }
        }

   
        if(muons.size() == 1 && met.p4.Perp() < 20 && ainvmasscutW && jets_size[i] >= 1 && datasets[d].Name() != "Jet" && datasets[d].Name() != "DataMu"){
	     if (muons[0].RelIso03PF() < 0.8){ 
                  IsoLooseForFakeRateNJets_Corr[i]->Fill(jets_Loose.size(),weight);
                  IsoLooseForFakeRateEta_Corr[i]->Fill(muons[0].p4.Eta(),weight);
                  IsoLooseForFakeRatePT_Corr[i]->Fill(muons[0].p4.Perp(),weight);
                  IsoLooseForFakeRateNVTX_Corr[i]->Fill(selVertices.size());
             }
   	     if (muons[0].RelIso03PF() < 0.2){ 
                  IsoTightForFakeRateNJets_Corr[i]->Fill(jets_Tight.size(),weight);
                  IsoTightForFakeRateEta_Corr[i]->Fill(muons[0].p4.Eta(),weight);
                  IsoTightForFakeRatePT_Corr[i]->Fill(muons[0].p4.Perp(),weight);
       ...
       if(muons.size() == 2 && datasets[d].Name() == "DataMu"){
        TLorentzVector Dilepton;
        Dilepton = muons[0].p4 + muons[1].p4;
        bool ainvmasscut = (Dilepton.M() < 106 && Dilepton.M() > 76);

        if(muons[0].Charge == -1 && muons[0].RelIso03PF() < 0.2){
          if(muons[1].Charge == +1 && ainvmasscut){
            if (muons[1].RelIso03PF() < 0.8){ 
                  IsoLooseForEfficiencyNJets->Fill(jets_Loose.size());
                  IsoLooseForEfficiencyEta->Fill(muons[1].p4.Eta());
                  IsoLooseForEfficiencyPT->Fill(muons[1].p4.Perp());
                  IsoLooseForEfficiencyNVTX->Fill(selVertices.size());
            }
	    if (muons[1].RelIso03PF() < 0.2){ 
                  IsoTightForEfficiencyNJets->Fill(jets_Tight.size());
                  IsoTightForEfficiencyEta->Fill(muons[1].p4.Eta());
                  IsoTightForEfficiencyPT->Fill(muons[1].p4.Perp());
                  IsoTightForEfficiencyNVTX->Fill(selVertices.size());
            }
          }
        }
        if(muons[1].Charge == -1 && muons[1].RelIso03PF() < 0.2){
          if(muons[0].Charge == +1 && ainvmasscut){
            if (muons[0].RelIso03PF() < 0.8){
                  IsoLooseForEfficiencyNJets->Fill(jets_Loose.size());
                  IsoLooseForEfficiencyEta->Fill(muons[0].p4.Eta());
                  IsoLooseForEfficiencyPT->Fill(muons[0].p4.Perp());
                  IsoLooseForEfficiencyNVTX->Fill(selVertices.size());
            }
 	    if (muons[0].RelIso03PF() < 0.2){
                  IsoTightForEfficiencyNJets->Fill(jets_Tight.size());
                  IsoTightForEfficiencyEta->Fill(muons[0].p4.Eta());
                  IsoTightForEfficiencyPT->Fill(muons[0].p4.Perp());
                  IsoTightForEfficiencyNVTX->Fill(selVertices.size());
            }
          }
        }
  ...
  TFile* file_MC = new TFile("MatrixMethod_Efficiency_MC_Mu.root");
  file_MC->cd();
 ...
 TFile* file_MM = new TFile("MatrixMethod_Efficiency_DATA_Mu.root","RECREATE");
  file_MM->cd();
