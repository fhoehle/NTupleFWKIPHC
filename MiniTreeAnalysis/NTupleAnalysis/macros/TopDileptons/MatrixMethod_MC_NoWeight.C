...
int main ()
{
   ...
   unsigned int nEvents = (int) (datasets[d].eventTree ()->GetEntries ());
    ...
    for (unsigned int ievt = 0; ievt < nEvents; ievt++) {
     ...
     double random = randomize.Uniform(1.);
       MMestEE.CountNSel(sel_aux, datasets[d], selTable_ee, d, 1, "ee", MMselStepCut);
       MMestMuMu.CountNSel(sel_aux, datasets[d], selTable_mumu, d, 1, "mumu", MMselStepCut);
       MMestEE.FillMMExpected(sel_aux, datasets[d], selTable_ee, d, 1, "ee", MMselStepCut, event);
       MMestMuMu.FillMMExpected(sel_aux, datasets[d], selTable_mumu, d, 1, "mumu", MMselStepCut, event);
  ...
  MMestEE.WriteMMFile("MatrixMethod_OutPut_EECase_MC_NoWeight.root");
  MMestMuMu.WriteMMFile("MatrixMethod_OutPut_MuMuCase_MC_NoWeight.root");
