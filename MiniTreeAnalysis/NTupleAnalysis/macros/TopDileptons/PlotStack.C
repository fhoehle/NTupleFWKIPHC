PlotStack(TString plotname, TString namechan, TString selection, bool setlogy, bool ratio){
  
  float lumi_error=0.036;
  ...
  */
  ...
  filename="backup_outputProof12-03-12_15-39-15/proof.root";
  ...
  if (namechan=="all") {
    ...
    h_2_DYToLL_M10_50           = (TH1F*)filechannel->Get(h_name2_DYToLL_M10_50);
  ...
  if(readStop){
    ...
    histo_STOP                  = (TH1F*)filechannel->Get(histo_STOP_name);
    if (namechan=="all") {
  ...
  latex2->DrawLatex(0.87, 0.95, "2.3 fb^{-1} at #sqrt{s} = 7 TeV");
  ...
  text2 = new TLatex(0.45,0.98, info_data);
  text2->SetNDC();
