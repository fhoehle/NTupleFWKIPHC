MatrixMethod_Results_EpsilonsTest_MuMu(string selection, string eventType){
  ...
  TFile * file_Estimated  = new TFile("MatrixMethod_OutPut_MuMuCase_MC_Fast_EpsilonsTest.root");
  ...
  if( selection == "Tight"){
    ...
    if(eventType == "Signal"){
      TCanvas * canv = new TCanvas(("canvEpsilonsTest_MuMu_"+selection+eventType).c_str(), ("canvEpsilonsTest_MuMu_"+selection+eventType).c_str(), 600, 800);
