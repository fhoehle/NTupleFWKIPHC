...
class PLRMeasurement{
  ...
  public:
	...
	void AddNuisanceParameter(const string& name, bool isGlobal, bool WithGraph, const float& Estimation, const float& ErrNeg, const float& ErrPos, const int& type);
	...
	//--- Main methods ---//
	float ComputeLikelihood(vector<bool> fixParam, vector<double> ParamValues, bool WriteSysName = false, bool doCorrelationMatrixHisto = false, bool verbose = false);//return the -minLogLik
	...
	void PLRValuesWithPol3Fit(TGraph* graph, int NofPoints, double* X_PLR, const double& Estimation, double& ErrNeg, double& ErrPos, double& Min, bool verbose = false, bool fit = false, bool fitrange = false, float fmin = -1., float fmax = 1.);
  ...
  private:
	...
	//--- Estimation ---//
	double MinLogLikStat; //at minimum of the PLR curve (stat only)
	double MinLogLik; //at minimum of the PLR curve (stat+syst)
	double MinFCN;	//during the minimization
	int NofFreeParam;
