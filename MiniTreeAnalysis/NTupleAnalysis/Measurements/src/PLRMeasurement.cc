...
void PLRMeasurement::AddNuisanceParameter(const string& name, bool isGlobal, bool WithGraph, const float& Estimation, const float& ErrNeg, const float& ErrPos, const int& type){
	//take care:: isGlobal means it affect globally the signal
	Parameter par;
	ResetParameter(par);
	par.isANuisanceParameter = true;
	...
	par.range = pair<double,double>(Estimation-5*ErrNeg,Estimation+5*ErrPos);
	par.name = name;
...
float PLRMeasurement::ComputeLikelihood (vector < bool > fixParam, vector < double >ParamValues, bool WriteSysName, bool doCorrelationMatrixHisto, bool verbose)
{
...
void PLRFcn (Int_t & npar, Double_t * gin, Double_t & f, Double_t * par, Int_t iflag)
{
  ...
  if (debug) {
	...
	for(unsigned int h=0;h<myPLRMeas->GetChannelsAddress()[c].GetHistosAddress().size();h++){
			...
			vector<double> uncertNorm; // uncertainty is modelized with 3 points and an interpolation - use the normalisation but not the shape
			vector<double> uncertShape; // uncertainty is modelized with 3 points and an interpolation - use the shape but not the normalisation
			...
			for(unsigned int u=0;u<myPLRMeas->GetParametersAddress().size();u++){
				...
				if(myPLRMeas->GetParametersAddress()[u].name!=string("Lumi") && myPLRMeas->GetParametersAddress()[u].name!=string("SignalXsection") && !myPLRMeas->GetParametersAddress()[u].isNormalization && !myPLRMeas->GetParametersAddress()[u].isGlobal){
					if(myPLRMeas->GetParametersAddress()[u].isANuisanceParameter){
						if(!myPLRMeas->GetParametersAddress()[u].WithGraph){
							if(myPLRMeas->GetParametersAddress()[u].type == 1){
								//cout<<"type 1 "<<myPLRMeas->GetParametersAddress()[u].name<<endl;
								if(!myPLRMeas->GetParametersAddress()[u].fixed) uncertNorm.push_back(par[u]);
								else uncertNorm.push_back(myPLRMeas->GetParametersAddress()[u].ExpectedValue);//0.0000 
							}
							if(myPLRMeas->GetParametersAddress()[u].type == 2){
								//cout<<"type 2 "<<myPLRMeas->GetParametersAddress()[u].name<<endl;
								if(!myPLRMeas->GetParametersAddress()[u].fixed) uncertShape.push_back(par[u]);
								else uncertShape.push_back(myPLRMeas->GetParametersAddress()[u].ExpectedValue);//0.0000 
							}
							if(myPLRMeas->GetParametersAddress()[u].type == 0){
								//cout<<"type 0"<<myPLRMeas->GetParametersAddress()[u].name<<endl;
  			...
  			if(debug){
				...
				for(unsigned int x=0;x<uncertNorm.size();x++){
					cout<<uncertNorm[x]<<endl;
				}
				for(unsigned int x=0;x<uncertShape.size();x++){
					cout<<uncertShape[x]<<endl;
				}
			...
			for(unsigned int id=0;id<myPLRMeas->GetDatasets().size();id++){
				...
				if(!myPLRMeas->GetDatasets()[id].isData() && myPLRMeas->GetDatasets()[id].Name().find(myPLRMeas->GetSignalDatasetName())>myPLRMeas->GetDatasets()[id].Name().size()){
					for(unsigned int u=0;u<myPLRMeas->GetParametersAddress().size();u++){
						//find parameters with the datasetname &
						if(myPLRMeas->GetParametersAddress()[u].name.find(myPLRMeas->GetDatasets()[id].Name())<myPLRMeas->GetParametersAddress()[u].name.size()){
						   ...
						   if(myPLRMeas->GetParametersAddress()[u].name.find(myPLRMeas->GetChannelsAddress()[c].GetChannelName())<myPLRMeas->GetParametersAddress()[u].name.size()){
							if(myPLRMeas->GetParametersAddress()[u].isCommon){
								...
								iu = u;
								for(unsigned int u2=0;u2<myPLRMeas->GetParametersAddress().size();u2++){
									if(myPLRMeas->GetParametersAddress()[u2].name.find(myPLRMeas->GetDatasets()[id].Name())<myPLRMeas->GetParametersAddress()[u2].name.size())
						   				if(myPLRMeas->GetParametersAddress()[u2].name.find(myPLRMeas->GetChannelsAddress()[0].GetChannelName())<myPLRMeas->GetParametersAddress()[u2].name.size())
											if(debug) cout<<"Use "<<myPLRMeas->GetParametersAddress()[u2].name<<" in channel "<<myPLRMeas->GetChannelsAddress()[0].GetChannelName()<<" as reference parameter"<< endl;
											use_ratio = true;
											ratio = par[u2]/myPLRMeas->GetParametersAddress()[u2].ExpectedValue;
								}
								break;
							}
							else{
					...
					if(iu>0){
						 if(!use_ratio) norm.push_back(par[iu]);  //non common parameter
						 else norm.push_back(par[iu]*ratio);
					}
		...
		if(nofdim == 2 ){
			...
			TH2F* hExp = myPLRMeas->GetChannelsAddress()[c].GetHistosAddress()[h].Get2DNexp(uncert, uncertNorm, uncertShape, GraphUncert, estim, GraphEstim, norm, LumiFactor, SignalFactor,myPLRMeas->GetSignalDatasetIteratorForHisto());
    ...
    for (unsigned int i = 0; i< myPLRMeas->GetParametersAddress().size(); i++) {
      if(myPLRMeas->GetParametersAddress()[i].isANuisanceParameter && !myPLRMeas->GetParametersAddress()[i].fixed){
		...
		if(!myPLRMeas->GetParametersAddress()[i].isGlobal){ LnLikelihood -= 0.5 * pow((par[i] - myPLRMeas->GetParametersAddress()[i].ExpectedValue),2.) / (pow((myPLRMeas->GetParametersAddress()[i].uncertainty.first+myPLRMeas->GetParametersAddress()[i].uncertainty.second)/2.,2.));
       			if(debug) cout<<"II "<<myPLRMeas->GetParametersAddress()[i].name<<" "<<myPLRMeas->GetParametersAddress()[i].isZeroCentered<<" "<<par[i]<<" "<<myPLRMeas->GetParametersAddress()[i].ExpectedValue<<" "<<myPLRMeas->GetParametersAddress()[i].uncertainty.first<<endl;
		}
		else{
				...
				for(unsigned int k=0;k<myPLRMeas->GetSignalSystematics()[j].size();k++){
					if(myPLRMeas->GetSignalSystematics()[j][k].name == myPLRMeas->GetParametersAddress()[i].name){
						...
						LnLikelihood -= 0.5 * pow((par[i] - 1.0),2.) / (pow((myPLRMeas->GetSignalSystematics()[j][k].effm1s+myPLRMeas->GetSignalSystematics()[j][k].effp1s)/2.,2.));
       						if(debug) cout<<"II "<<myPLRMeas->GetParametersAddress()[i].name<<" "<<myPLRMeas->GetParametersAddress()[i].isZeroCentered<<" "<<par[i]<<" "<<myPLRMeas->GetParametersAddress()[i].ExpectedValue<<" "<<myPLRMeas->GetSignalSystematics()[j][k].effm1s<<endl;
						}
					}
				}
...
void PLRMeasurement::RunLikelihood (bool doMCPseudoExp, int NofPseudoExp, bool SmearSystInPseudoExp, int verbosity, bool SkipOneByOne)
{
  ...
  for (int k = 0; k < NofPseudoExp; ++k) {
	...
	//cout<<"THEREEEEEEEEEEEEEEEEEEEEEEEEEE"<< GetSignalDatasetIteratorForHisto().size()<<endl;
  ...
  for(unsigned int i=0;i<GetEstimators().size();i++){
	...
	//explicitely change for btagging
        xmin = 145.;
	step = (double)((195.-145.)/GetEstimators()[i].NofPoints);
	//
	//double step = 0.05;
  ...
  // Now search the minimium of the PLR
   if(GetEstimators().size()==1){
	...
	//Determine roughtly the minimum
	PLRValuesWithPol3Fit(StatGraph1D, size, Xval, Estimation, ErrN, ErrP, Min);
	//Refit
	PLRValuesWithPol3Fit(StatGraph1D, size, Xval, Estimation, ErrN, ErrP, Min, false, false, true, Estimation-ErrN,Estimation+ErrP);
  ...
  for (int i = 0; i < SizeOfLoop; i++) {
   ...
   if(GetEstimators().size()==1){
	...
	//Refit
	PLRValuesWithPol3Fit(SystGraph1D, size, Xval, Estimation, ErrN, ErrP, Min,false, false, true, Estimation-ErrN,Estimation+ErrP);
	cout<<"Result of the PLR-1D minimization: "<<endl;
	cout<<"Estimation: "<<Estimation<<" - "<<ErrN<<" + "<< ErrP<< " ( Min: "<<Min<<" )"<<endl; 
	//Write the results
	double StatEstim = 0;
  ...
  //}//end of loop over Pseudo-Experiments
	...
	//---------------------------//
	MinLogLik = ComputeLikelihood (fixParam, ParamValues, false, false, verboseMax);
	//--- Save the results ---//
	count = 0;
	for(unsigned int i=0;i<parameters.size();i++){
		if(parameters[i].isANuisanceParameter){ 
...
void PLRMeasurement::PLRValuesWithPol3Fit (TGraph* g, int NofPoints, double *X_PLR,  const double& Estimation, double &ErrNeg, double &ErrPos, double &Min, bool verbose, bool print, bool fitrange, float fxmin, float fxmax)
{
  double YMini = 0.;
  ...
  if(print){
	 ...
	 g->Draw("AC");
  	 c.SaveAs("toto.png");
         ...
         if(verbose){
		if(fitrange)  g->Fit(&fmin,"QF","", fxmin, fxmax); 
		...
		for(unsigned int j=0; j<GetChannelsAddress()[i].GetBkgEstimation().size();j++){
			...
			if(GetChannelsAddress()[i].GetBkgEstimation()[j].isASyst){
				...
				if(GetChannelsAddress()[i].GetBkgEstimation()[j].isDDEstimated){
					...
					par.step = 0.1;
					double rmin = par.value-5*GetChannelsAddress()[i].GetBkgEstimation()[j].ErrNeg;
					if(rmin<0) rmin = 0.;
					par.range = pair<double,double>(rmin,par.value+5*GetChannelsAddress()[i].GetBkgEstimation()[j].ErrPos);
					parameters.push_back(par);
				}
				else{
					//here: problem ...
