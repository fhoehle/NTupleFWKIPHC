...
float DistributionDescription::GetFactorFromFitFunctionOfTheGraph(const TGraphErrors* graph, float xValue, string MethodName, float YThreshold) const{
	...
	if(graph->GetListOfFunctions()->GetSize()>=1){
	   	for(int i=0;i<graph->GetListOfFunctions()->GetSize();i++){
			...
			if(NewFactor==0){
				 if(printmessage) cout<<"DistributionDescription::"<<MethodName<<":: factor from the fit function should not be null - "<<message<<endl;
				 NewFactor = 1;
				 error = true;
			} 
			if(NewFactor<0){
				 if(printmessage) cout<<"DistributionDescription::"<<MethodName<<":: factor from the fit function should not be negative - "<<message<<endl;
				 NewFactor = 1;
				 error = true;
			} 
			if(NewFactor>YThreshold){ 
				if(printmessage) cout<<"DistributionDescription::"<<MethodName<<":: factor from the fit function is high - "<<message<<endl;
				error = true;
			}
			if(TMath::IsNaN(NewFactor)){
				if(printmessage) cout<<"DistributionDescription::"<<MethodName<<":: factor frome the fit function is NAN  - "<<message<<endl;
				NewFactor = 1;
				error = true;
			}
	...
	if(printmessage) cout<<"DistributionDescription::"<<MethodName<<":: there are not fit functions associated to the graph "<<graph->GetName()<<endl;
...
TH2F* DistributionDescription::Get2DNexp(const vector<double>& parUnc, const vector<double>& parNormUnc, const vector<double>& parShapeUnc, const vector<double>& GraphUnc, const vector<double>& parEstim, const vector<double>& GraphEstim, const vector<double>& bkgNorm, const double& LumiFactor, const double& SignalFactor, const vector<int>& iSignalDatasets) const{
	if((parUnc.size()+parNormUnc.size()+parShapeUnc.size())!=Histo_2D_UncertMinus.size() || (parUnc.size()+parNormUnc.size()+parShapeUnc.size())!=Histo_2D_UncertPlus.size()){
		cerr<<"DistributionDescription::Get2DNexp:: inconsistency in the size of the collection "<<endl;
		cerr<<"\t parUnc.size = "<<parUnc.size()<<"\t parNormUnc.size = "<<parNormUnc.size()<<"\t parShapeUnc.size = "<<parShapeUnc.size()<<" Histo_2D_UncertMinus.size = "<<Histo_2D_UncertMinus.size()<<" Histo_2D_UncertPlus.size = "<<Histo_2D_UncertPlus.size()<<endl;
	}
	if(GraphUnc.size()!=Graph_Uncert.size()){
		cerr<<"DistributionDescription::Get2DNexp:: inconsistency in the size of the collection "<<endl;
		cerr<<"\t GraphUnc.size = "<<GraphUnc.size()<<" Graph_Uncert.size = "<<Graph_Uncert.size()<<endl;
	...
	//-- Check that the fit of sfb/sfl doesn't change the integral of the distribution and correct for this effect
	float** paramCorrTab = new float*[GraphUnc.size()];
	for(unsigned int i=0;i<GraphUnc.size();i++) paramCorrTab[i] = new float[NofExpectedEvtsInit_2D.size()];
	for(unsigned int i=0;i<GraphUnc.size();i++){
		for(unsigned int d=0;d<NofExpectedEvtsInit_2D.size();d++){
	 	 float newint = 0.;
	 	 for(int ibin=1;ibin<NofExpectedEvtsSumCurrent_2D->GetNbinsX()+1;ibin++){
				...
				if(Graph_Uncert[i][d][graphIndice]){
					...
					newint+=NewFactor*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin);
				}
			}
	    	 }
		 float paramCorr = (float) NofExpectedEvtsInit_2D[d]->Integral()/newint;
		 //cout<<"paramCorr = "<<paramCorr<<endl;
		 paramCorrTab[i][d]=paramCorr;
	  	}
	}
	//------------------------------
	for(int ibin=1;ibin<NofExpectedEvtsSumCurrent_2D->GetNbinsX()+1;ibin++){
	  for(int jbin=1;jbin<NofExpectedEvtsSumCurrent_2D->GetNbinsY()+1;jbin++){
		...
		for(unsigned int d=0;d<NofExpectedEvtsInit_2D.size();d++){
			...
			float UncertaintyCorr = 1.;//0
			for(unsigned int i=0;i<parUnc.size();i++){
				if(!DatasetIsDD[d]) UncertaintyCorr*=Interpol(parUnc[i], 1., Histo_2D_UncertMinus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin), Histo_2D_UncertPlus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
			}
			for(unsigned int i=0;i<parNormUnc.size();i++){
				float tmp = Interpol(parNormUnc[i], 1., Histo_2D_UncertMinus[i][d]->Integral()/NofExpectedEvtsInit_2D[d]->Integral(), Histo_2D_UncertPlus[i][d]->Integral()/NofExpectedEvtsInit_2D[d]->Integral());
				if(!DatasetIsDD[d]) UncertaintyCorr*=Interpol(parNormUnc[i], 1., Histo_2D_UncertMinus[i][d]->Integral()/NofExpectedEvtsInit_2D[d]->Integral(), Histo_2D_UncertPlus[i][d]->Integral()/NofExpectedEvtsInit_2D[d]->Integral());
				//cout<<"parNorm: "<<parNormUnc[i]<<" "<<tmp<<endl;
			}
			for(unsigned int i=0;i<parShapeUnc.size();i++){
				float ratio1 = NofExpectedEvtsInit_2D[d]->Integral()/Histo_2D_UncertMinus[i][d]->Integral();
				float ratio2 = NofExpectedEvtsInit_2D[d]->Integral()/Histo_2D_UncertPlus[i][d]->Integral();
				float tmp = Interpol(parShapeUnc[i], 1., ratio1*Histo_2D_UncertMinus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin), ratio2*Histo_2D_UncertPlus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
				//cout<<"parShape: "<<parShapeUnc[i]<<" "<<tmp<<endl;
				UncertaintyCorr*=Interpol(parShapeUnc[i], 1., ratio1*Histo_2D_UncertMinus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin), ratio2*Histo_2D_UncertPlus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
			}
			for(unsigned int i=0;i<parEstim.size();i++){
				UncertaintyCorr*=Interpol(parEstim[i], 1., Histo_2D_EstimMinus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin), Histo_2D_EstimPlus[i][d]->GetBinContent(ibin,jbin)/NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
			}
			...
			for(unsigned int i=0;i<GraphUnc.size();i++){
				if(Graph_Uncert[i][d][graphIndice]){
					//float NewFactor = GetFactorFromFitFunctionOfTheGraph(Graph_Uncert[i][d][graphIndice], GraphUnc[i], string("Get2DNexp"));
					float NewFactor = GetFactorFromFitFunctionOfTheGraph(Graph_Uncert[i][d][graphIndice], GraphUnc[i], string("Get2DNexp"))*paramCorrTab[i][d];
					UncertaintyCorr*=NewFactor;
				}
			}
			...
			if(!DatasetIsDD[d]){
				...
				value+=LumiFactor*factor*UncertaintyCorr*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin);
				//*/
				//if we don't treat the Nuisance Parameters (UncertaintyCorr)
				/*
				NofExpectedEvtsFinal_2D[d]->SetBinContent(ibin,jbin, LumiFactor*factor*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
				if(isnan(UncertaintyCorr)) UncertaintyCorr = 1.;
				value+=LumiFactor*factor*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin);
				*/
			}
			//do not rescale to luminosity if the dataset is Data Driven
			else{
				//we treat the Nuisances Parameters for DD bkg
				if(isnan(UncertaintyCorr)) UncertaintyCorr = 1.;
				NofExpectedEvtsFinal_2D[d]->SetBinContent(ibin,jbin, factor*UncertaintyCorr*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
				value+=factor*UncertaintyCorr*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin);
				//if we don't treat the Nuisances Parameters for DD bkg
				/*
				NofExpectedEvtsFinal_2D[d]->SetBinContent(ibin,jbin, factor*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin));
				value+=factor*NofExpectedEvtsInit_2D[d]->GetBinContent(ibin,jbin);
				*/
			}
		}
