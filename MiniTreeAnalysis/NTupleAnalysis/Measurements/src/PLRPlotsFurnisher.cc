...
PLRPlotsFurnisher::PLRPlotsFurnisher(){
	...
	recognizedSystList.push_back(string("Q2"));
	//
	...
	for(unsigned int d=0;d<datasets.size();d++){
		for(unsigned int c=0; c<channels.size(); c++){
			for(unsigned int o=0; o<obs.size(); o++){
				...
				if(obs[o].dim == 1){
					MyhistoManager.CreateHisto(RefHistoCollection.GetHisto1D(c,o,0,0),hname,datasets[d].Name(),obs[o].axisName[0],Yaxis,obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second);
				}
				if(obs[o].dim == 2){
					 MyhistoManager.CreateHisto2D(RefHistoCollection.GetHisto2D(c,o,0,0),hname,datasets[d].Name(),obs[o].axisName[0],obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second,obs[o].axisName[1],obs[o].NofBins[1],obs[o].axisRange[1].first,obs[o].axisRange[1].second);
				}
				/////////////////////////////////
					...
					if(systList[s].discretized){
						...
						if(obs[o].dim == 1){
							// 0 correspond to minus and 1 to plus
							 MyhistoManager.CreateHisto(histoCollection.GetHisto1D(c,o,s,0),hnameMinus,datasets[d].Name(),obs[o].axisName[0],Yaxis,obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second);
							 MyhistoManager.CreateHisto(histoCollection.GetHisto1D(c,o,s,1),hnamePlus,datasets[d].Name(),obs[o].axisName[0],Yaxis,obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second);
						}
						if(obs[o].dim == 2){
							 MyhistoManager.CreateHisto2D(histoCollection.GetHisto2D(c,o,s,0),hnameMinus,datasets[d].Name(),obs[o].axisName[0],obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second,obs[o].axisName[1],obs[o].NofBins[1],obs[o].axisRange[1].first,obs[o].axisRange[1].second);
							 MyhistoManager.CreateHisto2D(histoCollection.GetHisto2D(c,o,s,1),hnamePlus,datasets[d].Name(),obs[o].axisName[0],obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second,obs[o].axisName[1],obs[o].NofBins[1],obs[o].axisRange[1].first,obs[o].axisRange[1].second);
						}
					}
					else{
						if(obs[o].dim == 1 && (systList[s].name==string("sfb") || systList[s].name==string("sfl")) ){
							 //take care ... works only for b-tagging
						   	//cout<<"Create plots for syst "<<systList[s].name<<endl;
							 string hnameV = hname + "_0b";
							 MyhistoManager.CreateHisto2D(histoCollection.GetHisto2D(c,o,s,0),hnameV,datasets[d].Name(),obs[o].name,obs[o].axisName[0],systList[s].values.size(),systList[s].values[0]-(systList[s].values[1]-systList[s].values[0])/2.,systList[s].values[systList[s].values.size()-1]+(systList[s].values[1]-systList[s].values[0])/2.,obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second);
							 hnameV = hname + "_1b";
							 MyhistoManager.CreateHisto2D(histoCollection.GetHisto2D(c,o,s,0),hnameV,datasets[d].Name(),obs[o].name,obs[o].axisName[0],systList[s].values.size(),systList[s].values[0]-(systList[s].values[1]-systList[s].values[0])/2.,systList[s].values[systList[s].values.size()-1]+(systList[s].values[1]-systList[s].values[0])/2.,obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second);
							 hnameV = hname + "_2b";
							 MyhistoManager.CreateHisto2D(histoCollection.GetHisto2D(c,o,s,0),hnameV,datasets[d].Name(),obs[o].name,obs[o].axisName[0],systList[s].values.size(),systList[s].values[0]-(systList[s].values[1]-systList[s].values[0])/2.,systList[s].values[systList[s].values.size()-1]+(systList[s].values[1]-systList[s].values[0])/2.,obs[o].NofBins[0],obs[o].axisRange[0].first,obs[o].axisRange[0].second);
						...
						else cerr<<"PLRPlotsFurnisher::CreateHistos:: Dim 2 is not implemented there !!"<<endl; 
					}
				}
			}
		}
...
//   if ( ChannelName=="ee" ) std::cout<<"code de schtroumpf before "<<weight<<std::endl;
   ...
   if ( ChannelName=="mumu" &&  !dataset.isData() ) weight*=sf_trig_mu;	
   if ( ChannelName=="ee" &&  !dataset.isData() )   weight*=sf_trig_ee;	
   if ( ChannelName=="emu" &&  !dataset.isData() )  weight*=sf_trig_emu;	
   
   if ( ChannelName == "ee" &&  !dataset.isData() ){
      if ( dataset.Name()=="TTbar")  {
   	// ee
   	if (tmeme == 2 || tmeme == 10101 || tmeme == 20200  )   weight = weight*sf_e*sf_e;
   	// emu
   	if ( tmeme==11 || tmeme==21100 || tmeme==11001 || tmeme==10110 ) weight = weight*sf_e;
   	// ejet
   	if (tmeme == 1 || tmeme == 10100) weight = weight*sf_e;
       }
       if ( dataset.Name()=="Zjets" 	     || 
   	  dataset.Name()=="DYToEE_M-20"	   || 
   	  dataset.Name()=="DYToTauTau_M-20"    || 
   	  dataset.Name()=="DYToEE_M-10To20"    || 
   	  dataset.Name()=="DYToTauTau_M-10To20"
   	  )weight = weight*sf_e*sf_e;
       if ( dataset.Name()=="WZ" || dataset.Name()=="WW" || dataset.Name()=="ZZ"  || dataset.Name()=="VV" )weight = weight*sf_e*sf_e;
       if ( dataset.Name()=="WjetsE"  || dataset.Name()=="WjetsTau" || dataset.Name()=="Wjets") weight = weight*sf_e;
       if ( dataset.Name()=="SingleToptW" || dataset.Name()=="TtW" || dataset.Name()=="TbartW") weight = weight*sf_e*sf_e;
   }
   ...
   if(ChannelName == "emu" &&  !dataset.isData() ){
      if ( dataset.Name()=="TTbar")  {
   	...
   	if (tmeme == 2 || tmeme == 10101 || tmeme == 20200  ) {
   	      weight = weight*sf_e;
   	      if (nrecoel>1) weight = weight*sf_e;
   	}
   	// emu
   	if ( tmeme==11 || tmeme==21100 || tmeme==11001 || tmeme==10110 ) weight = weight*sf_e;
   	// ejet
...
// 	 if ( dataset.Name()=="WjetsE"  || dataset.Name()=="WjetsTau" ) --> hyp : real mu, fake e
       if ( dataset.Name()=="SingleToptW" || dataset.Name()=="TtW" || dataset.Name()=="TbartW") weight = weight*sf_e;
   }
   
//   std::cout<<"CandType0 "<<CandType0<<" "<<LeptonSF<<std::endl;
   if (CandType0=="mumu" &&  !dataset.isData() && (  dataset.Name() != "Wjets" &&  dataset.Name() != "QCD"  
      && !(dataset.Name()  =="TTbar" &&  !IsSignal) )   ) {
//    || (dataset.Name()  =="TTbar" &&  IsSignal)  )   ) {
       weight*= LeptonSF;
   }
   if (CandType0=="emu"  &&  !dataset.isData() 
      && ( dataset.Name() != "Wjets" &&  dataset.Name() != "QCD"  
      && !(dataset.Name()  =="TTbar" &&  !IsSignal) )   ) {
//    || (dataset.Name()  =="TTbar" &&  IsSignal)	  ) ){
       weight*= LeptonSF;
   }
...
// NB SF_mu pas code dans MyCutFlow.C (car ==1): il n'apparait pas ici non plus	
   ...
   if (ChannelName=="mumu"){ 
      if (itypemc==2 || itypemc==6 || itypemc==7 )   {weight*=sf_dy_mm; }
   }
   else if (ChannelName=="emu" ){
      if (itypemc==2 || itypemc==6 || itypemc==7 )    {weight*=sf_dy_em; }
   }
   else if (ChannelName=="ee"  ){
      if (itypemc==2 || itypemc==6 || itypemc==7 )    {weight*=sf_dy_ee; }
   }
//endmodifdg
	
	//Produce reference plots
	for(unsigned int c=0;c<channels.size();c++){
		if(ChannelName!=channels[c]) continue;
		...
		if(selected){
			for(unsigned int o=0;o<obs.size();o++){
				...
				if(obs[o].name == string("NjetsNBjets") ){
				   if(!dataset.isData()){
					//0 b-jet
					...
					MyhistoManager.FillHisto2D(RefHistoCollection.GetHisto2D(c,o,0,0),hname,systSel.GetJetMultiplicity(), systSel.GetNofBtaggedJets(), DatasetName, IsSignal, weight);
				   }
				}	
			}
		}
//	if ( ChannelName=="ee" ) std::cout<<"code de schroumf after "<<weight<<std::endl;	
 	...
 	//test: removed to avoid crash
        if(dataset.isData()) return; //don't compute any systematic
					...
					if(selected){
						for(unsigned int o=0;o<obs.size();o++){
							...
							if(obs[o].name == string("Count") || obs[o].name == string("Njets")){
								float yvalue = -1.;
								if(obs[o].name == string("Count")) yvalue = 0.;
								if(obs[o].name == string("Njets")) yvalue = systSel.GetJetMultiplicity();
...
int PLRPlotsFurnisher::GetiDataset(string DatasetName){
	for(unsigned int i=0;i<datasets.size();i++){
		if(datasets[i].Name()==DatasetName) return (int)i;
	}
	return -1;
}
