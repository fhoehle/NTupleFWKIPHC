  ...
  while (elem)
	...
	if( elem->Value() == string("NuisanceParameters")){
		...
		while(NParamElem){
			...
			NParamElem->QueryIntAttribute("type",&type);
			if(doIt>0){
				cout<<"name: "<<name<<" Expectation: "<<ExpectedValue<<" ErrNeg: "<<ErrNeg<<" ErrPos: "<<ErrPos<<endl; 
				...
				if(ExpectedValue>-999 && ErrNeg>-999 && ErrPos>-999) plr.AddNuisanceParameter(name, false, WithGraph, ExpectedValue, ErrNeg, ErrPos, type);
				else plr.AddNuisanceParameter(name, false, WithGraph);
			}
...
void AnalysisEnvironmentLoader::LoadWeight (DiLeptonSelection& sel)
{
  ...
  if(!elem) return;
      ...
      int methodb =0;
      int systb =0;
      string file1("efficacite_btag.root");
      string file2("eff_from_ttmadgraph_summer11_multiwp.root");
      string file3("sf_ttbar_BTV-11-003.txt");
      if (type == string ("Btag")) {
      ...
      sel.InitSFBWeight(flag, methodb, systb, btagAlgo, btagDiscriCut, btagNjet, file1, file2, file3);
