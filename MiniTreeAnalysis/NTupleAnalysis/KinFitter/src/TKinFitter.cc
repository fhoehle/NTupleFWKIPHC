...
void TKinFitter::addMeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    std::cout <<"NullPointer : " << "Measured particle points to NULL. It will not be added to the KinFitter."<<std::endl;
  }
...
void TKinFitter::addUnmeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    std::cout << "NullPointer : " << "Unmeasured particle points to NULL. It will not be added to the KinFitter."<< std::endl;
  }
...
Double_t TKinFitter::getF() {
  ...
  for (unsigned int indexConstr = 0; indexConstr < _constraints.size(); indexConstr++) {
    ...
    //std::cout << "_constraints[indexConstr]->getCurrentValue() "<<_constraints[indexConstr]->getCurrentValue()<<std::endl;
  }
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString name) {
  ...
  if (!matrix.IsValid()) {
    std::cout << "InvalidMatrix : " << "Matrix " << name << " is invalid."<<std::endl;
    return;
  }
