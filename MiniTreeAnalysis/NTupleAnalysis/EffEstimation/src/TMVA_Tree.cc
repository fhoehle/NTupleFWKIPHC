...
////////:to run on MC
void TMVA_Tree::FillHistos(std::vector<NTJet> theJets, NTMET Met, std::vector<NTMuon> selMuons, std::vector<NTElectron> selElectrons,
std::vector<NTTau> theTausLoose, std::vector<NTTau> theTausMedium, bool PassTriggerQuadJet40IsoPFTauSelection,  const float& weight){
	...
	for(unsigned int j=0;j<theJets.size();j++){
		...
		if (theJets[j].p4.Pt()>45) njets_45++;
  ...
  if ( theJets.size() >= 4 && njets_45>=3 &&  
        theTausMedium.size() == 1 && Met.p4.Et()>15 &&
        PassTriggerQuadJet40IsoPFTauSelection == true 
        && selMuons.size()==0 && selElectrons.size()==0 )//&& btagjets.size()>=1)
 {
