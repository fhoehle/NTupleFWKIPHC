...
//Jets selection
//************************************************  
std::vector<NTJet> Selection::GetCleanJet(std::vector<NTJet> jet_cand, std::vector<NTMuon> muon_cand, std::vector<NTElectron> elec_cand){

  std::vector<NTJet> cleanJets;
 
  for(unsigned int i=0;i<jet_cand.size();i++){
      if(fabs(jet_cand[i].p4.Eta())> JetEtaThreshold_ ||  jet_cand[i].p4.Pt()<JetPtThreshold_) continue;
	...
	if(deltaR < deltaRel) deltaRel = deltaR;
      ...
      //if( deltaRmu > 0.4  ) selectedJets.push_back(jet_cand[i]);
...
std::vector<NTJet> Selection::GetScaledJets(float scale) const{
    ...
    for (unsigned int i=0; i<newJets.size(); i++){
      ...
      unc->setJetEta(newJets[i].p4.Eta());
       ...
       if ( scale<1.0 ) {
          newscale = (1.- unc->getUncertainty(false));
       } else {
          newscale = (1.+ unc->getUncertainty(true));
       }
      //std::cout<<"eta,pt, newscale, uncert "<<eta<<" "<<pt<<" "<<1+totalJECunc->getUncertainty(true)<<" "<<newscale<<std::endl;
      //std::cout<<"eta,pt, scale "<<eta<<" "<<pt<<" "<<newscale<<std::endl;
      double Px = 0;
      double Py = 0;
      double Pz = 0;
      double E = 0;
...
std::vector<NTJet>  Selection::GetSmearedJets(vector<NTJet> injets, float jetResFactor) const{
    ...
    for (unsigned int i=0; i<newJets.size(); i++){
      ...
      if(fabs(newJets[i].p4Gen.Eta()) < 1.1 ) jetResFactor = 0.05;
      if(fabs(newJets[i].p4Gen.Eta()) < 1.7 && fabs(newJets[i].p4Gen.Eta()) > 1.1 ) jetResFactor = 0.10;
      if(fabs(newJets[i].p4Gen.Eta()) > 1.7 ) jetResFactor = 0.13;
...
NTMET Selection::GetScaledMET(float scale) const{
    double newMetX = met.p4.Px();
    double newMetY = met.p4.Py();
    std::vector<NTJet> scaledJets = GetScaledJets(scale);

    for (unsigned int i=0; i<jets.size(); i++){
      if ((fabs(jets[i].p4.Eta())< 5.0) && (jets[i].p4.Pt()>15.0) && 
		looseJetId(jets[i]) && cleanJet(jets[i], muons, electrons)) {
	newMetX += (jets[i].p4.Px()-scaledJets[i].p4.Px());
	newMetY += (jets[i].p4.Py()-scaledJets[i].p4.Py());
      }
    }

    NTMET newMET = met;
    newMET.p4.SetPxPyPzE(newMetX, newMetY, 0 , sqrt(pow(newMetX,2.) + pow(newMetY,2.) + met.p4.M()));
    return newMET;
}
...
NTMET Selection::GetUnclusScaledMET(bool applyUnclusScale, float scale) const{
    ...
    NTMET newMET = met;
    newMET.p4.SetPxPyPzE(met.p4.Px()+(scale-1)*unclusMET.Px(), met.p4.Py()+(scale-1)*unclusMET.Py(), 0 , sqrt(pow(met.p4.Px()+(scale-1)*unclusMET.Px(),2.) + pow(met.p4.Py()+(scale-1)*unclusMET.Py(),2.) + met.p4.M()));
    return newMET;
}
...
bool Selection::looseJetId(const NTJet & theJet) const
{
  ...
  //if( theJet.chargedMultiplicity()<=0 && fabs(theJet.eta())<2.4 ) 
    //ThisIsClean=false;

  // always require #Constituents > 1
  //if( theJet.n90Hits() <=1 ) ThisIsClean=false;
...
std::vector<NTJet> Selection::GetSelectedJets(const std::vector<NTMuon> & muon_cand,
	const std::vector<NTElectron> & elec_cand, bool applyJES, float scale, bool applyJER, float ResFactor) const{
  ...
  for(unsigned int i=0;i<scaledJets.size();i++){
    ...
    bool ThisIsClean=true;
    //apply following only if |eta|<2.4: CHF>0, CEMF<0.99, chargedMultiplicity>0   
    if(( scaledJets[i].CHEF)<= 0.0  
       && fabs(scaledJets[i].p4.Eta())<2.4) ThisIsClean=false; 
    if( (scaledJets[i].CEEF)>= 0.99 
        && fabs(scaledJets[i].p4.Eta())<2.4 ) ThisIsClean=false;
	
    //if( scaledJets[i].chargedMultiplicity()<=0 && fabs(scaledJets[i].eta())<2.4 ) 
      //ThisIsClean=false;

    // always require #Constituents > 1
    //if( scaledJets[i].n90Hits() <=1 ) ThisIsClean=false;

    if(ThisIsClean && 
       (scaledJets[i].NHEF)< 0.99 
       && (scaledJets[i].NEEF)<0.99) 
    passingLoose=true;

    if(!passingLoose) continue;
...
{
  return (getBtagDiscr(jet, algo) >= discricut);
}

double Selection::getBtagDiscr(const NTJet & jet, const int& algo) const
{
  switch (algo) {
    case 0:
...
/*std::vector<NTElectron> Selection::GetScaledElectrons(float scale) const{
    ...
    return GetScaledElectrons(scale, electrons);
}*/
     
/*std::vector<NTElectron> Selection::GetScaledElectrons(float factor,
	std::vector<NTElectron> newElectrons) const{
...
std::vector<NTElectron> Selection::GetSelectedElectronsNoIso(float PtThr, float EtaThr, bool applyLES, float scale,
  ...
  //if(applyLES ){ localElectrons = GetScaledElectrons(scale, localElectrons); }
...
void Selection::InitSFBWeight (int flagb, int methodb, int systb, int btagAlgo_,
	float btagDiscriCut_, int NofBtagJets_, string & file1, string & file2, string & file3)
{
...
void Selection::LoadElScaleFactors(){
  string fileName(getenv( "CMSSW_BASE" )+string("/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/electronSF.root"));
...
void Selection::LoadMuScaleFactors(){
  string fileName(getenv( "CMSSW_BASE" )+string("/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/muonSF.root"));
...
void Selection::InitJESUnc (char* jecContrib)
{
  ...
  string fileName = getenv( "CMSSW_BASE" )+string("/src/MiniTreeAnalysis/NTupleAnalysis/macros/data/JEC11_V12_AK5PF_UncertaintySources.txt");
