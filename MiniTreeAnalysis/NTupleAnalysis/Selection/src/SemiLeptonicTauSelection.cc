...
SemiLeptonicTauSelection::SemiLeptonicTauSelection()
{
      ...
      TFile* f3 = new  TFile("/opt/sbg/data/data1/cms/cferro/official8/CMSSW_4_2_5/src/MiniTreeAnalysis/NTupleAnalysis/macros/GOOD_Turnon/TriggerEffHisto_TauLeg40_simmetricBinMultiJet_Full_QuadJet40.root");
      f3->cd();
      Tau40_Leg= (TH1F*)f3->Get("eff_tau");
      
      TFile* f4 = new  TFile("/opt/sbg/data/data1/cms/cferro/official8/CMSSW_4_2_5/src/MiniTreeAnalysis/NTupleAnalysis/macros/GOOD_Turnon/TriggerEffHisto_TauLeg45_GOOD.root");
      f4->cd();
      ...
      reader->BookMVA("MLP","/opt/sbg/data/data1/cms/lebihan/official8/CMSSW_4_2_5/src/MiniTreeAnalysis/NTupleAnalysis/macros/weights/TMVAClassification_MLP_KinFit_MET15_cris_looseTau.weights.xml");
...
int SemiLeptonicTauSelection::doFullSelection(Dataset* dataset, string channelName){
	...
	if(dataset->Name() == "TTbarLL"       && 
	  (tmeme == 2  || tmeme == 10101 || tmeme == 20200 || 
       ...
       tau_cand  = GetSelectedTaus(muon_cand, elec_cand, false ,1.0,0,true);
       else
       tau_cand  = GetSelectedTaus(muon_cand, elec_cand, false,1,0,true);
       tausAna = tau_cand;
	...
	for (unsigned int j=0;j<jet_cand.size();j++)
	    { if (jet_cand[j].p4.Pt()>45) njets_45++;}
	...
	if((dataset->Name().find("DATA")!=std::string::npos && (int) btagjets.size()>=1) || dataset->Name().find("DATA")==std::string::npos) {
...
int SemiLeptonicTauSelection::FillTable(SelectionTable& selTable, Dataset* dataset, int idataset, float weight){
