...
void TauHistoManager::CreateHistos(){
	...
	AddHisto(string("Pt"),string("p_{T}(tau)"),string("p_{T}(tau)"),100,0,200);	
	...
	AddHisto(string("leadTrackPt"),string("leadTrackPt(tau)"),string("leadTrackPt(tau)"),40,0,200);
	...
	AddHisto(string("MT"),string("M_{T}(#tau,MET)"),string("M_{T}(#tau,MET)"),25,0,250);
	...
	AddHisto(string("DeltaPhiTauMet"),string("#Delta #phi (#tau,MET)"),string("#Delta #phi (#tau,MET)"),20,0,4);
	AddHisto(string("DeltaPhiTauMetNN"),string("#Delta #phi (#tau,MET) NN>0.5"),string("#Delta #phi (#tau,MET) NN>0.5"),20,0,4);
...
void TauHistoManager::Fill(NTMET met, const vector<NTTau>& taus, const vector<NTVertex>& vertices, const int& maxSelStep, const int& iChannel, const int& iDataset, const float& weight, const float& NNOutput){
	if(!Check(iChannel, iDataset)) return;
	for(unsigned int i=0;i<SelectionSteps.size();i++){
		if(maxSelStep>=(int)i) FillSelStep(met, taus, vertices, i, iChannel, iDataset, weight, NNOutput);	
	}	
}

void TauHistoManager::FillSelStep(NTMET met, const vector<NTTau>& taus, const vector<NTVertex>& vertices, const int& iSelStep, const int& iChannel, const int& iDataset, const float& weight, const float& NNOutput){
